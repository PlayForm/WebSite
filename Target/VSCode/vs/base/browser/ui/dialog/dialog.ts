...
export interface IDialogOptions {
	...
	readonly buttonOptions?: Array<
		| undefined
		| { sublabel?: string; styleButton?: (button: IButton) => void }
	>;
...
export class Dialog extends Disposable {
	...
	constructor(
		...
		if (this.options.renderFooter) {
			this.footerContainer = this.element.appendChild(
				$(".dialog-footer-row"),
			);
			...
			const buttonBar = (this.buttonBar = this._register(
				new ButtonBar(this.buttonsContainer, {
					alignment:
						...
						DialogContentsAlignment.Vertical
			...
			this._register(
				addDisposableListener(
					...
					(e) => {
						...
						if (
							isMacintosh &&
							evt.equals(KeyMod.CtrlCmd | KeyCode.KeyD)
						) {
							EventHelper.stop(e);
							...
							const noButton = buttonMap.find(
								(button) =>
									button.index === 1 &&
									button.index !== this.options.cancelId,
							);
							if (noButton) {
								resolve({
						...
						if (
							...
							if (this.footerContainer) {
								const links =
									this.footerContainer.querySelectorAll("a");
								for (const link of links) {
									focusableElements.push(link);
									if (isActiveElement(link)) {
										focusedIndex =

...
export function isAuthorizationDynamicClientRegistrationResponse(
	...
	return (
		response.client_id !== undefined && response.client_name !== undefined
	);
}
...
export function isAuthorizationDeviceTokenErrorResponse(
	obj: unknown,
): obj is IAuthorizationDeviceTokenErrorResponse {
	if (typeof obj !== "object" || obj === null) {
		return false;
	}
	const response = obj as IAuthorizationDeviceTokenErrorResponse;
	return (
		response.error !== undefined && response.error_description !== undefined
	);
}
...
export function getClaimsFromJWT(token: string): IAuthorizationJWTClaims {
	...
	try {
		...
		if (typeof decodedHeader !== "object") {
			throw new Error(
				"Invalid JWT token format: header is not a JSON object",
			);
		}
		...
		if (typeof decodedPayload !== "object") {
			throw new Error(
				"Invalid JWT token format: payload is not a JSON object",
			);
		}

...
export async function findExecutable(
	...
	cwd?: string,
	paths?: string[],
	env: Platform.IProcessEnvironment = process.env as Platform.IProcessEnvironment,
	...
	for (const pathEntry of paths) {
		...
		if (Platform.isWindows) {
			const pathExtsFound = getWindowPathExtensions(env).map(
				async (ext) => {
					const withExtension = fullPath + ext;
					return (await fileExists(withExtension))
						? withExtension
						: undefined;
				},
			);

...
class Renderer
	implements ICompressibleTreeRenderer<Element, void, HTMLElement>
{
...
suite("AsyncDataTree", function () {
	...
	test("issue #80098 - first expand should call getChildren", async () => {
		return runWithFakedTimers({ useFakeTimers: true }, async () => {
			...
			assert.strictEqual(
				race,
				"timeout",
				"expand(a) should not be yet done",
			);
	...
	test("issue #199441", async () => {
		...
		const compressionDelegate = new (class
			implements ITreeCompressionDelegate<Element>
		{
	...
	test("Tree Navigation: AsyncDataTree", async () => {
		...
		assert.strictEqual(
			tree.navigate(model.get("b")).previous()?.id,
			"ab.txt",
		);
		assert.strictEqual(tree.navigate(model.get("b")).next()?.id, "ba");

		assert.strictEqual(
			tree.navigate(model.get("ab.txt")).previous()?.id,
			"ab",
		);
	...
	test("Test Navigation: CompressibleAsyncDataTree", async () => {
		...
		assert.strictEqual(
			tree.navigate(model.get("ba")).previous()?.id,
			"aa.txt",
		);
		...
		assert.strictEqual(
			tree.navigate(model.get("aa.txt")).previous()?.id,
			"aa",
		);

...
suite("Fuzzy Scorer", () => {
	...
	test("scoreItem - ensure upper case bonus only applies on non-consecutive matches (bug #134723)", function () {
		...
		assert.ok(
			scoreItem(resourceAllLower, "asdf", true, ResourceAccessor).score >
				scoreItem(resourceWithUpper, "asdf", true, ResourceAccessor)
					.score,
		);
	});
	...
	test("fuzzyScore2 (multiple queries)", function () {
		...
		function assertScore() {
			assert.ok(
				multiScore ??
					0 >= (firstSingleScore ?? 0) + (secondSingleScore ?? 0),
			);
			for (let i = 0; multiMatches && i < multiMatches.length; i++) {
				const multiMatch = multiMatches[i];
				const firstAndSecondSingleMatch =
					firstAndSecondSingleMatches[i];

...
suite("observables", () => {
	...
	suite("observableReducer", () => {
		test("main", () => {
			...
			const myObservable1 = observableValue<number, number>(
				"myObservable1",
				5,
			);
			const myObservable2 = observableValue<number, number>(
				"myObservable2",
				9,
			);
		...
		const signal2Derived = derivedHandleChanges(
			...
			(reader: IDerivedReader<{ message: string }>, changeSummary) => {
				for (const c of changeSummary.changes) {
					reader.reportChange({
						message: c.change.message + " (derived)",
					});
				}
			},
		);

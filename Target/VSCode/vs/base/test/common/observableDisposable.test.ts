...
} from "../../common/observableDisposable.js";
...
suite("ObservableDisposable", () => {
	...
	suite("• addDisposable()", () => {
		...
		test("• disposes the entire tree of disposables", async () => {
			...
			const allDisposableObjects = disposableObjects(
				disposablesCount,
				object,
			);
			...
			// finally validate that all objects are disposed
			const allDisposed = allDisposableObjects.reduce(
				(acc, disposable) => {
	...
	suite("• asserts", () => {
		test("• not disposed (method)", async () => {
			// this is an abstract class, so we have to create
			// an anonymous class that extends it
			const object: ObservableDisposable =
				new (class extends ObservableDisposable {})();
			disposables.add(object);

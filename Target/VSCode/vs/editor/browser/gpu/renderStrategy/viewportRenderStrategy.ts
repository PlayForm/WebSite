...
export class ViewportRenderStrategy extends BaseRenderStrategy {
	...
	private _cellBindBufferLineCapacity =
		Constants.CellBindBufferInitialCapacity;
	...
	private _rebuildCellBuffer(lineCount: number) {
		...
		const lineCountWithIncrement =
			(Math.floor(lineCount / Constants.CellBindBufferCapacityIncrement) +
				1) *
			...
			for (
				...
				for (x = tokenStartIndex; x < tokenEndIndex; x++) {
					...
					if (
						...
						charWidth =
							this.glyphRasterizer.getTextMetrics(chars).width;
					}
					...
					if (chars === " " || chars === "\t") {
						...
						if (chars === "\t") {
							...
							absoluteOffsetX +=
								charWidth * (tabXOffset - offsetBefore);

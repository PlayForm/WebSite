...
export class ObservableCodeEditor extends Disposable {
	...
	private constructor(public readonly editor: ICodeEditor) {
		...
		>(
			{
				owner: this,
				equalsFn: equalsIfDefined(
					itemsEquals(Selection.selectionsEqual),
				),
				lazy: true,
			},
		...
		this.cursorPosition = derivedOpts(
			...
			(reader) =>
				this.selections.read(reader)?.[0]?.getPosition() ?? null,
		);
		...
		this.layoutInfo = observableFromEvent(
	...
	public createContentWidget(widget: IObservableContentWidget): IDisposable {
		const contentWidgetId =
			"observableContentWidget" + this._widgetCounter++;

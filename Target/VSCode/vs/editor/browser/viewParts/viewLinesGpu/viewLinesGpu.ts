...
export class ViewLinesGpu extends ViewPart implements IViewLines {
	...
	async initWebgpu() {
		...
		this._device =
			ViewGpuContext.deviceSync || (await ViewGpuContext.device);
	...
	getPositionAtCoordinate(
		...
		const spaceWidthDevicePixels =
			this._lastViewLineOptions.spaceWidth * dpr;
		...
		for (let x = 0; x < content.length; x++) {
			...
			if (chars === "\t") {
				...
				charWidth =
					spaceWidthDevicePixels * (tabXOffset - offsetBefore);

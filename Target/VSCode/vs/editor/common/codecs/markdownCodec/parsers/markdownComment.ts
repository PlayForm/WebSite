	...
	public accept(
		...
		if (token instanceof Dash) {
			...
			if (lastToken instanceof Dash) {
				const token1: TSimpleDecoderToken | undefined =
					this.currentTokens[0];
				const token2: TSimpleDecoderToken | undefined =
					this.currentTokens[1];
				const token3: TSimpleDecoderToken | undefined =
					this.currentTokens[2];
				const token4: TSimpleDecoderToken | undefined =
...
> {
	...
	private get endsWithDashes(): boolean {
		...
		const secondLastToken =
			this.currentTokens[this.currentTokens.length - 2];

...
import {
	FrontMatterMarker,
	TMarkerToken,
} from "../tokens/frontMatterMarker.js";
import {
	assertNotConsumed,
	IAcceptTokenSuccess,
	ParserBase,
...
export class PartialFrontMatterStartMarker extends ParserBase<
	TMarkerToken,
	PartialFrontMatterStartMarker | PartialFrontMatterHeader
> {
...
export class PartialFrontMatterHeader extends ParserBase<
	TSimpleDecoderToken,
	PartialFrontMatterHeader | FrontMatterHeader
> {
	/**
	...
	constructor(public readonly startMarker: FrontMatterMarker) {
		super([]);
	}

	public override get tokens(): readonly TSimpleDecoderToken[] {
		const endMarkerTokens =
			this.maybeEndMarker !== undefined ? this.maybeEndMarker.tokens : [];
	...
	public asFrontMatterHeader(): FrontMatterHeader {
		...
		assert(
			this.maybeEndMarker.dashCount ===
				this.startMarker.dashTokens.length,
			[
	...
	private acceptEndMarkerToken(
		...
		if (result === "success") {
			...
			const endMarkerParsingComplete =
				nextParser instanceof FrontMatterMarker;
...
class PartialFrontMatterEndMarker extends ParserBase<
	TMarkerToken,
	PartialFrontMatterEndMarker | FrontMatterMarker
> {
	constructor(token: Dash) {
		const { range } = token;
	...
	public accept(
		token: TSimpleDecoderToken,
	): TAcceptTokenResult<PartialFrontMatterEndMarker | FrontMatterMarker> {
		const previousToken = this.currentTokens[this.currentTokens.length - 1];

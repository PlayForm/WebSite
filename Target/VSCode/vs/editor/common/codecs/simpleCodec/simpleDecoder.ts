...
import {
	...
	TLineBreakToken,
	TLineToken,
} from "../linesCodec/linesDecoder.js";
import {
	At,
	Tab,
	Word,
 ...
 */
export const WELL_KNOWN_TOKENS: readonly ISimpleTokenClass<TSimpleToken>[] =
	Object.freeze([
		LeftParenthesis,
		RightParenthesis,
		LeftBracket,
...
> {
	...
	protected override onStreamData(line: TLineToken): void {
		...
		while (i < lineText.length) {
			...
			if (tokenConstructor) {
				this._onData.fire(
					SimpleToken.newOnLine(line, columnNumber, tokenConstructor),
				);
			...
			while (
				i < lineText.length &&
				!WORD_STOP_CHARACTERS.includes(lineText[i])
			) {
				word += lineText[i];
				i++;
			}

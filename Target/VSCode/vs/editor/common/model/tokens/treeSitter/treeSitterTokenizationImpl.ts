...
export class TreeSitterTokenizationImpl extends Disposable {
	...
	constructor(
		...
		this._tokenStore.buildStore(
			this._createEmptyTokens(),
			TokenQuality.None,
		);
	...
	public tokenizeLinesAt(
		...
		const rawLineTokens = this._guessTokensForLinesContent(
			lineNumber,
			lines,
		);
	...
	private _guessTokensForLinesContent(
		...
		for (let i = 0; i < lines.length; i++) {
			...
			for (
				let j = tokensIndex;
				!moveToNextLine && j < tokens.length;
				j++
			) {
				const token = tokens[j];
				const lineAdjustedEndOffset = token.endOffset - lineStartOffset;
				const lineAdjustedStartOffset =
					tokenStartOffset - lineStartOffset;
			...
			lineStartOffset +=
				lines[i].length + this._textModel.getEOL().length;
		}
	...
	private _handleTreeUpdate(ranges: RangeChange[], versionId: number) {
		...
		for (let i = 0; i < ranges.length; i++) {
			const rangeLinesLength =
				...
				ranges[i].newRange.startLineNumber;
			if (rangeLinesLength > chunkSize) {
				...
				do {
					...
					rangeChanges.push({
						...
						endOffset: this._textModel.getOffsetAt(
				...
				if (
					i === 0 ||
					rangeChanges[i - 1].endOffset <
						ranges[i].newRangeStartOffset
				) {
					rangeChanges.push({
			...
			this._onDidChangeTokens.fire({
				changes: {
					...
					ranges: [
						{
							fromLineNumber:
								range.range.getStartPosition().lineNumber,
							toLineNumber:
								range.range.getEndPosition().lineNumber,
						},
					],
				},
			});
	...
	private _rangeTokensAsUpdates(
		...
		for (const token of endOffsetToken) {
			if (
				...
				(startingOffsetInArray &&
					token.endOffset < startingOffsetInArray)
			) {
				continue;
			}
		...
		for (let i = 0; i < captures.length; i++) {
			...
			const startLine =
				...
				capStartLine < range.endLineNumber
					...
					: range.startLineNumber;
			const endLine =
				capEndLine > range.startLineNumber &&
	...
	private _createTokensFromCaptures(
		...
		const endOffsetsAndScopes: EndOffsetAndScopes[] = Array(
			captures.length,
		);
		...
		const addCurrentTokenToArray = (
			...
			if (position !== undefined) {
				...
				const prevEndOffset =
					position > 0
						? endOffsetsAndScopes[position - 1].endOffset
						: 0;
			...
			if (previousEndOffset >= endOffset) {
				...
				do {
					...
					previousTokenStartOffset =
						withinTokenIndex >= 1
							? endOffsetsAndScopes[withinTokenIndex - 1]
									.endOffset
							: 0;
					previousTokenEndOffset =
						withinTokenIndex >= 0
		...
		if (endOffsetsAndScopes[tokenIndex - 1].endOffset < rangeLength) {
			if (
				rangeLength - endOffsetsAndScopes[tokenIndex - 1].endOffset >
				0
			) {
				increaseSizeOfTokensByOneToken();
				endOffsetsAndScopes[tokenIndex] = {

...
import {
	rangesIntersect,
	rangesEqual,
} from "./treeSitterSyntaxTokenBackend.js";
...
export class TreeSitterTree extends Disposable {
			...
			(this._ranges &&
				this._ranges.every(
					(range) =>
						...
						newTree.rootNode.startPosition.row,
				)) ||
		...
		do {
			if (newCursor.currentNode.hasChanges) {
				...
				const changedChildren = newChildren.filter((c, index) => {
					if (
						c?.hasChanges ||
						oldCursor.currentNode.children.length <= index
					) {
				...
				if (
					...
					newCursor.currentNode.hasError !==
						oldCursor.currentNode.hasError
				) {
	...
	private _findTreeChanges(
		...
		for (let nodeIndex = 0; nodeIndex < changedNodes.length; nodeIndex++) {
			...
			const cursorContainersNode = () =>
				cursor.startIndex < node.startIndex &&
				cursor.endIndex > node.endIndex;
			...
			if (
				...
				if (
					newRanges[newRangeIndex].startIndex <
				...
				if (
					newRanges[newRangeIndex].endIndex >
			...
			if (
				...
				// Merge the changes
				mergedChanges[mergedChanges.length - 1].newRange =
					Range.fromPositions(
						mergedChanges[
							mergedChanges.length - 1
	...
	private _constrainRanges(changes: RangeChange[]): RangeChange[] {
		...
		while (
			...
			rangesIndex < this._ranges.length
		) {

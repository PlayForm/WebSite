	...
	public getAccumulatedLineHeightsIncludingLineNumber(
		...
		return (
			...
			this._defaultLineHeight *
				(lineNumber - previousSpecialLine.lineNumber)
		);
	}
		...
		if (
			...
			for (let i = 0; i < this._orderedCustomLines.length; i++) {
				...
				if (i < startIndexOfDeletion) {
					...
					i >= startIndexOfDeletion &&
					i <= endIndexOfDeletion
				) {
	...
	public onLinesInserted(fromLineNumber: number, toLineNumber: number): void {
		...
		if (decorationsWithGaps.size > 0) {
			for (const decorationId of decorationsWithGaps) {
				const decoration =
					this._decorationIDToCustomLine.get(decorationId);
				if (decoration) {
	...
	public commit(): void {
		...
		for (const pendingChange of this._pendingSpecialLinesToInsert) {
			const candidateInsertionIndex =
				this._binarySearchOverOrderedCustomLinesArray(
			...
			if (
				...
				if (previousSpecialLine) {
					prefixSum =
						...
						previousSpecialLine.maximumSpecialHeight +
						this._defaultLineHeight *
							(customLine.lineNumber -
								previousSpecialLine.lineNumber -
								1);
				} else {
					prefixSum =
						this._defaultLineHeight * (customLine.lineNumber - 1);
				}

...
export class InlineCompletionsModel extends Disposable {
	...
	constructor(
		...
		this._fetchSpecificProviderSignal = observableSignal<
			InlineCompletionsProvider | undefined
		>(this);
		this._selectedInlineCompletionId = observableValue<string | undefined>(
			this,
			undefined,
		);
		this.primaryPosition = derived(
			this,
		...
		this._inlineSuggestMode = this._editorObs
			...
			.map((v) => v.mode);
		this._suppressedInlineCompletionGroupIds = this._editorObs
			.getOption(EditorOption.inlineSuggest)
		...
		this._didUndoInlineEdits = derivedHandleChanges(
			...
			(reader, changeSummary) => {
				...
				if (
					...
					this._lastAcceptedInlineCompletionInfo
						.textModelVersionIdAfter ===
						versionId - 1 &&
					this._lastAcceptedInlineCompletionInfo.inlineCompletion
						.isInlineEdit &&
					changeSummary.didUndo
				) {
					this._lastAcceptedInlineCompletionInfo = undefined;
				...
				changeTracker: {
					...
					handleChange: (ctx, changeSummary) => {
						...
						if (
							...
							ctx.didChange(this._forceUpdateExplicitlySignal)
						) {
							...
							ctx.didChange(this._onlyRequestInlineEditsSignal)
						) {
							changeSummary.onlyRequestInlineEdits = true;
						} else if (
							ctx.didChange(this._fetchSpecificProviderSignal)
						) {
			...
			(reader, changeSummary) => {
				...
				let context: InlineCompletionContextWithoutUuid = {
					...
					includeInlineCompletions:
						!changeSummary.onlyRequestInlineEdits,
				...
				const itemToPreserveCandidate =
					...
					this._inlineCompletionItems.get()?.inlineEdit;
		...
		this.selectedInlineCompletion = derived<
		...
		this.activeCommands = derivedOpts<Command[]>(
			{ owner: this, equalsFn: itemsEquals() },
			(r) =>
		...
		this.inlineCompletionsCount = derived<number | undefined>(
			this,
			(reader) => {
				if (
					...
					return this._filteredInlineCompletionItems.read(reader)
						.length;
				} else {
					return undefined;
				}
		...
		this._hasVisiblePeekWidgets = derived(
			this,
		...
		>(
			...
			(reader) => {
				...
				if (inlineEditResult) {
					...
					const commands =
						inlineEditResult.source.inlineSuggestions.commands;
				...
				if (suggestItem) {
					...
					const fullEdit =
						augmentation?.edit ?? suggestCompletionEdit;
		...
		this._tabShouldIndent = derived(this, (reader) => {
			...
			return selections?.some((s) => {
				if (s.isEmpty()) {
					return (
						this.textModel.getLineLength(s.startLineNumber) === 0
					);
				} else {
					return (
						isMultiLine(s) ||
		...
		this.tabShouldJumpToInlineEdit = derived(this, (reader) => {
			...
			if (
				this._inAcceptFlow.read(reader) &&
				appearedInsideViewport.read(reader)
			) {
				return false;
			}
		...
		this.tabShouldAcceptInlineEdit = derived(this, (reader) => {
			...
			if (
				this._jumpedToId.read(reader) === s.inlineCompletion.semanticId
			) {
				return true;
			}
			if (this._tabShouldIndent.read(reader)) {
				...
				const jumpToReset = this.state
					.map(
						(s, reader) =>
							!s ||
							(s.kind === "inlineEdit" &&
								!s.cursorAtInlineEdit.read(reader)),
					)
					.read(reader);
				if (jumpToReset) {
				...
				for (const provider of providers) {
					...
					store.add(
						provider.onDidChangeInlineCompletions(() => {
							...
							if (
								activeState &&
								(activeState.inlineCompletion ||
									activeState.edits) &&
								activeState.inlineCompletion?.source
									.provider !== provider
							) {
								return;
							}
							...
							transaction((tx) => {
								this._fetchSpecificProviderSignal.trigger(
									tx,
									provider,
								);
								this.trigger(tx);
	...
	private _lastShownInlineCompletionInfo:
		| {
				alternateTextModelVersionId: number /* already freed! */;
				inlineCompletion: InlineSuggestionItem;
		  }
		| undefined;
	private _lastAcceptedInlineCompletionInfo:
		| {
				textModelVersionIdAfter: number /* already freed! */;
				inlineCompletion: InlineSuggestionItem;
		  }
		| undefined;
	...
	public async trigger(
		tx?: ITransaction,
		options?: { onlyFetchInlineEdits?: boolean; noDelay?: boolean },
	): Promise<void> {
	...
	public jump(): void {
		...
		transaction((tx) => {
			...
			const isSingleLineChange =
				...
				(s.inlineCompletion.displayLocation ||

...
export class InlineCompletionsSource extends Disposable {
	...
	constructor(
		...
		super();
		this._updateOperation = this._register(
		...
		this.inlineCompletions = this._state.map(
			this,
			(v) => v.inlineCompletions,
		);
		this.suggestWidgetInlineCompletions = this._state.map(
			this,
	...
	public fetch(
		...
		if (
			!providerhasChangedCompletion &&
			this._updateOperation.value?.request.satisfies(request)
		) {
		...
		const promise = (async () => {
			...
			try {
				...
				let providerResult: InlineCompletionProviderResult | undefined =
					undefined;
...
class InlineCompletionsState extends Disposable {
	...
	public createStateWithAppliedResults(
		update: InlineCompletionProviderResult,
		request: UpdateRequest,
		textModel: ITextModel,
		itemToPreserve: InlineSuggestionIdentity | undefined,
	): InlineCompletionsState {
		...
		if (itemToPreserve) {
			...
			if (
				item &&
				!update.has(item.getSingleTextEdit()) &&
				item.canBeReused(textModel, request.position)
			) {
				items.unshift(item);
			}
		}

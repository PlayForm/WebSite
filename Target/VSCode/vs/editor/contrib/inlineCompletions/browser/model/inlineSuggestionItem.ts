...
abstract class InlineSuggestionItemBase {
	constructor(
		...
		public readonly displayLocation:
			| InlineSuggestDisplayLocation
			| undefined,
	) {}
	...
	public get isFromExplicitRequest(): boolean {
		return (
			this._data.context.triggerKind ===
...
class InlineSuggestDisplayLocation implements IDisplayLocation {
	...
	public withEdit(
		...
		if (
			!newOffsetRange ||
			newOffsetRange.length !== this._offsetRange.length
		) {
			return undefined;
		}
...
export class InlineCompletionItem extends InlineSuggestionItemBase {
	public static create(
		...
		const edit = reshapeInlineCompletion(
			new StringReplacement(
				transformer.getOffsetRange(data.range),
				insertText,
			),
			textModel,
		);
		...
		const displayLocation = data.displayLocation
			? InlineSuggestDisplayLocation.create(
	...
	override withEdit(
		...
		const newEdit = new StringReplacement(
			newEditRange[0],
			this._textEdit.text,
		);
		const positionOffsetTransformer =
			getPositionOffsetTransformerFromTextModel(textModel);
		const newTextEdit =
			positionOffsetTransformer.getSingleTextEdit(newEdit);
...
export class InlineEditItem extends InlineSuggestionItemBase {
	public static create(
		...
		const offsetEdit = getStringEdit(
			textModel,
			data.range,
			data.insertText,
		);
	...
	if (totalInnerEdits === 1) {
		...
		if (prefixLength + suffixLength === originalText.length) {
			return new StringReplacement(
				edit.replaceRange
					.deltaStart(prefixLength)
					.deltaEnd(-suffixLength),

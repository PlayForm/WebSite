...
export async function provideInlineCompletions(
	...
	function queryProviderOrPreferredProvider(
		...
		const deferredPromise = new DeferredPromise<
			InlineSuggestionList | undefined
		>();
		...
		(async () => {
			if (!circle) {
				const preferred = getPreferredProviders(provider);
				for (const p of preferred) {
					const result = await queryProviderOrPreferredProvider(
						p,
						states,
					);
...
async function addRefAndCreateResult(
	...
	for await (const completions of inlineCompletionLists) {
		...
		for (const item of completions.inlineSuggestionsData) {
			...
			itemsByHash.set(
				createHashFromSingleTextEdit(item.getSingleTextEdit()),
				item,
			);
...
function createHashFromSingleTextEdit(edit: TextReplacement): string {
	return JSON.stringify([
		edit.text,
		edit.range.getStartPosition().toString(),
	]);
}

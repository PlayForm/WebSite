	...
	constructor(
		...
		this.uiState = derived(this, (reader) => {
			...
			const edit = new StringEdit(
				inlineTexts.map((t) =>
					StringReplacement.insert(t.column - 1, t.text),
				),
			);
		...
		this.decorations = derived(this, (reader) => {
			...
			if (uiState.replacedRange) {
				decorations.push({
					...
					options: {
						inlineClassName:
							"inline-completion-text-to-replace" +
							extraClassNames,
					...
					options: {
						...
						after: {
							...
							inlineClassName:
								...
								p.lineDecorations
									.map((d) => " " + d.className)
									.join(" "), // TODO: take the ranges into account for line decorations
		...
		this._isInlineTextHovered = this._editorObs.isTargetHovered(
			(p) =>
				...
				p.target.detail.injectedText.options.attachedData.owner ===
					this,
			this._store,
		);
		...
		if (this._isClickable) {
			this._register(
				this._additionalLinesWidget.onDidClick((e) =>
			...
			this._register(
				this._editor.onMouseUp((e) => {
					...
					const a =
						e.target.detail.injectedText?.options.attachedData;
					if (
						a instanceof GhostTextAttachedData &&
						a.owner === this
					) {
				...
				this._editor.onBeforeExecuteEdit(
					(e) =>
						(this.hasBeenAccepted =
							e.source === "inlineSuggestion.accept"),
				),
			);
		}
...
function isTargetGhostText(target: EventTarget | null): boolean {
	return (
		...
		target.classList.contains(GHOST_TEXT_CLASS_NAME)
	);
}

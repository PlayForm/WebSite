...
export class InlineEditsGutterIndicator extends Disposable {
	...
	constructor(
		...
		this._lineNumberToRender = derived(this, (reader) => {
			...
			const lineNumber =
				this._originalRange.read(reader)?.startLineNumber;
			...
			if (
				lineNumberOptions.renderType === RenderLineNumbersType.Interval
			) {
			...
			if (
				lineNumberOptions.renderType === RenderLineNumbersType.Relative
			) {
				const cursorPosition =
					this._editorObs.cursorPosition.read(reader);
				if (!cursorPosition) {
		...
		this._layout = derived(this, (reader) => {
			...
			const iconDocked = derived((reader) => {
				...
				const editStartLineNumber =
					s.range.read(reader).startLineNumber;
			...
			if (pillIsPartiallyDocked) {
				...
				const pillRect = pillRectMoved.withWidth(
					iconWidth(pillRectMoved),
				);
				const iconRect = pillRect;
								...
								n.div(
									{
										...
										style: {
											...
											position: "absolute",
											backgroundColor:
												this._gutterIndicatorStyles.map(
													(v) => v.background,
												),
	...
	private _showHover(): void {
		...
		disposableStore.add(
			focusTracker.onDidBlur(() =>
				this._focusIsInMenu.set(false, undefined),
			),
		);
		disposableStore.add(
			focusTracker.onDidFocus(() =>
				this._focusIsInMenu.set(true, undefined),
			),
		);
		disposableStore.add(

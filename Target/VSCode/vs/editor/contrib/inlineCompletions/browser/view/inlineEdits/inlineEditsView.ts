...
export class InlineEditsView extends Disposable {
	...
	private _previousView:
		| {
				id: string;
				view: ReturnType<
					typeof InlineEditsView.prototype.determineView
				>;
				editorWidth: number;
				timestamp: number;
		  }
	...
	constructor(
		...
		this._indicator = derived<InlineEditsGutterIndicator | undefined>(
			...
			(reader) => {
				if (
					!this._indicatorCyclicDependencyCircuitBreaker.read(reader)
				) {
					return undefined;
				}
				...
				const indicatorDisplayRange = derivedOpts(
					...
					(reader) => {
						const ghostTextIndicator =
							this._ghostTextIndicator.read(reader);
						...
						if (state.state?.kind === "custom") {
							...
							if (!range) {
								throw new BugIndicatingError(
									"custom view should have a range",
								);
							}
		...
		this._gutterIndicatorOffset = derived<number>(this, (reader) => {
			...
			if (
				this._uiState.read(reader)?.state?.kind === "insertionMultiLine"
			) {
		...
		this._wordReplacementViews = mapObservableArrayCached(
			this,
			this._uiState.map((s) =>
				s?.state?.kind === "wordReplacements"
					? s.state.replacements
					: [],
			),
			(e, store) => {
				return store.add(
	...
	private determineView(
		...
		if (
			...
			if (
				isSingleLineInsertionAfterPosition(
					diff,
...
function isSingleMultiLineInsertion(diff: DetailedLineRangeMapping[]) {
	...
	if (
		...
		change.modifiedRange.endLineNumber
	) {
		return false;
	}

	...
	implements IInlineEditsView
{
	...
	private readonly _state = derived(this, (reader) => {
		...
		if (
			...
			const endOfLineColumn =
				textModel.getLineLength(state.lineNumber - 1) + 1;
			return {
	...
	private readonly _overlayLayout = derived(this, (reader) => {
		...
		const top =
			this._editor.getTopForLineNumber(state.lineNumber) -
			scrollTop +
			topTrim;
					...
					n.div({
						...
						style: {
							...
							border: getModifiedBorderColor(this._tabAction).map(
								(bc) =>
									`${BORDER_WIDTH}px solid ${asCssVariable(bc)}`,
							),
							boxSizing: "border-box",

	...
	constructor(
		...
		this._layout = derived(this, (reader) => {
			...
			const topOfOriginalLines =
				this._editor.editor.getTopForLineNumber(startLineNumber) -
				scrollTop;
			const bottomOfOriginalLines =
				this._editor.editor.getBottomForLineNumber(endLineNumber) -
				scrollTop;
		...
		>((reader) => {
			...
			const viewZoneLineNumber =
				edit.originalRange.endLineNumberExclusive;
			...
			.div(
				...
				[
					derived((reader) => {
						...
						const contentLeft =
							this._editor.layoutInfoContentLeft.read(reader);
		...
		this._register(
			this._editor.createOverlayWidget({
				...
				minContentWidthInPx: derived((reader) => {
					return (
						this._layout.read(reader)?.minContentWidthRequired ?? 0
					);
				}),
	...
	private removePreviousViewZone(changeAccessor: IViewZoneChangeAccessor) {
		...
		if (
			...
			this._editor.editor.setScrollTop(
				this._editor.scrollTop.get() -
					this._previousViewZoneInfo.height,
			);
		}

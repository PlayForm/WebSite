	...
	implements IInlineEditsView
{
	...
	constructor(
				...
				if (reducedLinesCount > 0) {
					this._activeViewZones.push(
						changeAccessor.addZone({
							...
							domNode: $(
								"div.diagonal-fill.inline-edits-view-zone",
							),
						}),
					);
				}
		...
		this._editorMaxContentWidthInRange = derived(this, (reader) => {
			const originalDisplayRange =
				this._originalDisplayRange.read(reader);
			...
			return derivedObservableWithCache<number>(
				this,
				(reader, lastValue) => {
		...
		this._previewEditorLayoutInfo = derived(this, (reader) => {
			...
			const horizontalScrollOffset =
				this._editorObs.scrollLeft.read(reader);
			...
			const maxPreviewEditorLeft = Math.max(
				...
				Math.min(
					cursorPos
						? getOffsetForPos(this._editorObs, cursorPos, reader) +
								50
						: 0,
			...
			.svg(
				...
				[
					n.svgElem("path", {
						...
						d: derived((reader) => {
							const layoutInfo =
								this._previewEditorLayoutInfo.read(reader);
							if (!layoutInfo) {
								return undefined;
							}
		...
		this._originalOverlay = n
			.div(
				...
				derived((reader) => {
					...
					const hasBorderLeft =
						layoutInfoObs.read(reader).codeScrollLeft !== 0;
					const isModifiedLower = layoutInfoObs.map(
						(layoutInfo) =>
							layoutInfo.codeRect.bottom <
							layoutInfo.editRect.bottom,
					);
					const transitionRectSize =
					...
					const overlayRect = layoutInfoObs.map((layoutInfo) =>
						layoutInfo.codeRect.intersectHorizontal(
							intersectionLine,
						),
					);

		...
		this._layout = derived(this, (reader) => {
			...
			const center = new Point(
				contentLeft +
					start.x +
					w / 2 -
					this._editor.scrollLeft.read(reader),
				start.y,
			);
				...
				[
					derived((reader) => {
						...
						const modifiedBorderColor = asCssVariable(
							getModifiedBorderColor(this._tabAction).read(
								reader,
							),
						);
							...
							n.svg(
								{
									...
									style: {
										...
										left: derived(
											(reader) =>
												layout.read(reader).center.x -
												9,
										),
										top: derived(
											(reader) =>
												layout.read(reader).center.y +
												4,
										),

...
function getUniqueGlyphId(): [
	...
	if (!lastUniqueGlyph) {
		...
		lastUniqueGlyph = String.fromCharCode(
			lastUniqueGlyph.charCodeAt(0) + 1,
		);
	}
...
suite("TextureAtlas", () => {
	...
	setup(() => {
		...
		atlas = store.add(
			instantiationService.createInstance(TextureAtlas, 2, undefined),
		);
	...
	test("get multiple glyphs", () => {
		atlas = store.add(
			instantiationService.createInstance(TextureAtlas, 32, undefined),
		);
		for (let i = 0; i < 10; i++) {
			assertIsValidGlyph(
				atlas.getGlyph(glyphRasterizer, ...getUniqueGlyphId()),
	...
	test("adding a glyph larger than the standard slab size", () => {
		...
		atlas = store.add(
			instantiationService.createInstance(TextureAtlas, 32, {
	...
	test("adding a non-first glyph larger than the standard slab size, causing an overflow to a new page", () => {
		atlas = store.add(
			instantiationService.createInstance(TextureAtlas, 2, {
				allocatorType: (canvas, textureIndex) =>
					new TextureAtlasSlabAllocator(canvas, textureIndex, {
						slabW: 1,
						slabH: 1,

 ...
 */
export type TTokensConsumeMethod =
	| "async-generator"
	| "consume-all-method"
	| "on-data-event";
...
> extends Disposable {
	...
	public sendData(inputData: string | string[]): this {
		...
		const interval = setInterval(() => {
			...
			const dataToSend = inputDataBytes.slice(
				0,
				randomInt(inputDataBytes.byteLength),
			);
	...
	public async run(
		...
		try {
			...
			const receivedTokens =
				await this.receiveTokens(tokensConsumeMethod);
	...
	private randomTokensConsumeMethod(): TTokensConsumeMethod {
		...
		switch (testConsumeMethodIndex) {
			...
			default: {
				throw new Error(
					`Unknown consume method index '${testConsumeMethodIndex}'.`,
				);
			}
		}
	}
	...
	public async receiveTokens(
		...
		switch (tokensConsumeMethod) {
			...
			default: {
				throw new Error(
					`Unknown consume method '${tokensConsumeMethod}'.`,
				);
			}
		}

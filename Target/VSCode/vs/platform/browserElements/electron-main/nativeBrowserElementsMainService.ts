...
export const INativeBrowserElementsMainService =
	createDecorator<INativeBrowserElementsMainService>(
	...
	implements INativeBrowserElementsMainService
{
	...
	async findWebviewTarget(
		...
		const { targetInfos } =
			await debuggers.sendCommand("Target.getTargets");
		let target: (typeof targetInfos)[number] | undefined = undefined;
		const matchingTarget = targetInfos.find(
				...
				try {
					...
					if (browserType === BrowserType.LiveServer) {
						return (
							...
							url.searchParams.get("extensionId") ===
	...
	async waitForWebviewTargets(
		...
		while (Date.now() - start < timeout) {
			const targetId = await this.findWebviewTarget(
				debuggers,
				windowId,
				browserType,
			);
	...
	async startDebugSession(
		...
		const simpleBrowserWebview = allWebContents.find(
			(webContent) => webContent.id === window.id,
		);
		...
		try {
			const matchingTargetId = await this.waitForWebviewTargets(
				debuggers,
				windowId!,
				browserType,
			);
		...
		window.win.webContents.on(
			"ipc-message",
			async (event, channel, closedCancelAndDetachId) => {
				if (
					channel ===
					`vscode:cancelCurrentSession${cancelAndDetachId}`
				) {
					...
					if (window.win) {
						window.win.webContents.removeAllListeners(
							"ipc-message",
						);
					}
				}
	...
	async getElementData(
		...
		try {
			...
			const { sessionId } = await debuggers.sendCommand(
		...
		const nodeData = await this.getNodeData(
			targetSessionId,
			debuggers,
			window.win,
			cancellationId,
		...
		return new Promise((resolve, reject) => {
			const onMessage = async (
				...
				if (method === "Overlay.inspectNodeRequested") {
					...
					if (!backendNodeId) {
						throw new Error(
							"Missing backendNodeId in inspectNodeRequested event",
						);
					}
					...
					try {
						...
						const { model } = await debuggers.sendCommand(
							"DOM.getBoxModel",
							{ nodeId },
							sessionId,
						);
			...
			window.webContents.on(
				"ipc-message",
				async (event, channel, closedCancellationId) => {
					if (
						channel ===
						`vscode:cancelElementSelection${cancellationId}`
					) {
			...
			for (const inherited of matched.inherited) {
				...
				for (const ruleEntry of rules) {
					...
					const selectors = rule.selectorList.selectors
						...
						.join(", ");
					lines.push(
						`/* Inherited from ancestor level ${level} (${rule.origin}) */`,
					);
					lines.push(`${selectors} {`);
					for (const prop of rule.style.cssProperties) {
						if (prop.name && prop.value) {

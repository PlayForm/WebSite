...
suite("Marker Service", () => {
	...
	test("resource filter hides markers for the filtered resource UNLESS explicit read", () => {
		...
		// Verify resource1 markers are visible again
		assert.strictEqual(
			service.read({ ignoreResourceFilters: true }).length,
			2,
		);
		assert.strictEqual(
			service.read({ resource: resource1, ignoreResourceFilters: true })
				.length,
			1,
		);
		assert.strictEqual(
		...
		assert.strictEqual(
			service.read({ resource: resource2, ignoreResourceFilters: true })
				.length,
			1,
		);
		assert.strictEqual(
	...
	test("resource filter affects all filter combinations", () => {
		...
		assert.strictEqual(
			service.read({ owner: "owner1", resource }).length,
			1,
		);
		assert.strictEqual(
			service.read({ severities: MarkerSeverity.Error }).length,
		...
		const ownerResourceMarkers = service.read({
			owner: "owner1",
			resource,
		});
		assert.strictEqual(ownerResourceMarkers.length, 1);
		assert.strictEqual(
			ownerResourceMarkers[0].severity,
			MarkerSeverity.Info,
		);
	...
	test("multiple filters for same resource are handled correctly", () => {
		...
		const filter2 = service.installResourceFilter(
			resource,
			"Second filter",
		);

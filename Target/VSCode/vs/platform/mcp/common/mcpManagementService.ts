	...
	implements IMcpManagementService
{
	...
	constructor(
		...
		@IMcpGalleryService
		private readonly mcpGalleryService: IMcpGalleryService,
	...
	async installFromGallery(
		...
		try {
			...
			const { userLocal } =
				this.configurationService.inspect<IMcpServersConfiguration>(
					"mcp",
				);
			...
			if (serverConfig.inputs) {
				...
				for (const input of serverConfig.inputs) {
					if (
						!value.inputs.some(
							(i) => (<IMcpServerVariable>i).id === input.id,
						)
					) {
						value.inputs.push({
							...input,
	...
	private getServerConfig(
		...
		if (packageType === undefined) {
			packageType =
				manifest.packages?.[0]?.registry_name ?? PackageType.REMOTE;
		}

...
export class QuickInputController extends Disposable {
	...
	constructor(
		...
		this.inQuickInputContext =
			InQuickInputContextKey.bindTo(contextKeyService);
	...
	private getUI(showInActiveContainer?: boolean): QuickInputUI {
					...
					switch (event.keyCode) {
						...
						case KeyCode.Tab:
							if (
								...
								if (
									event.shiftKey &&
									dom.isAncestor(event.target, stops[0])
								) {
									dom.EventHelper.stop(event, true);
		...
		this._register(
			autorun((reader) => {
				const dndViewState =
					this.dndController?.dndViewState.read(reader);
				if (!dndViewState) {
					return;
				}

				if (
					dndViewState.top !== undefined &&
					dndViewState.left !== undefined
				) {
					this.viewState = {
						...this.viewState,
...
class QuickInputDragAndDropController extends Disposable {
	...
	private _quickInputAlignmentContext: IContextKey<
	...
	constructor(
		...
		this._controlsOnLeft =
			customWindowControls && platform === Platform.Mac;
		...
		this._register(
			dom.addDisposableGenericMouseUpListener(
				...
				(event: MouseEvent) => {
					...
					if (
						!this._quickInputDragAreas.some(
							({ node, includeChildren }) =>
								includeChildren
									? dom.isAncestor(
											originEvent.target as HTMLElement,
											node,
										)
		...
		this._register(
			dom.addDisposableGenericMouseDownListener(
				...
				(e: MouseEvent) => {
					...
					const dragAreaRect =
						this._quickInputContainer.getBoundingClientRect();
					const dragOffsetX =
						originEvent.browserEvent.clientX - dragAreaRect.left;
					const dragOffsetY =
						originEvent.browserEvent.clientY - dragAreaRect.top;
	...
	private _layout(topCoordinate: number, leftCoordinate: number) {
		...
		topCoordinate = Math.max(
			...
			Math.min(
				topCoordinate,
				this._container.clientHeight -
					this._quickInputContainer.clientHeight,
			),
		);

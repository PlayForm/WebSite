...
export class ThemeMainService extends Disposable implements IThemeMainService {
	...
	private doUpdateWindowSplashOverride(
		...
		let workspaceOverride =
			splashOverride.layoutInfo.workspaces[workspace.id];
		if (!workspaceOverride) {
			...
			if (
				...
				ThemeMainService.WORKSPACE_OVERRIDE_LIMIT
			) {
				delete splashOverride.layoutInfo.workspaces[
					workspaceEntries[0]
				];
				changed = true;
			}
	...
	private doGetWindowSplash(
		...
		if (workspace) {
			if (
				override.layoutInfo.workspaces[workspace.id]?.sideBarVisible ===
				false
			) {
				sideBarWidth = 0;
			} else {
		...
		if (workspace) {
			const auxiliaryBarVisible =
				override.layoutInfo.workspaces[workspace.id]
					?.auxiliaryBarVisible;

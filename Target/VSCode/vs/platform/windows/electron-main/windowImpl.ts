		...
		this._register(
			Event.fromNodeEventEmitter(
				...
				if (
					...
					this.logService.debug(
						`Cleared maximized window ${this.id} state`,
					);
				}
		...
		if (isWindows && useCustomTitleStyle) {
			this._register(
				Event.fromNodeEventEmitter(
					win,
					"system-context-menu",
					(event: Electron.Event, point: Electron.Point) => ({
						event,
						point,
					}),
				)((e) => {
		...
		if (isWindows && this.environmentMainService.enableRDPDisplayTracking) {
			...
			this._register(
				Event.fromNodeEventEmitter(
					screen,
					"display-added",
					(event: Electron.Event, display: Display) => ({
						event,
						display,
					}),
				)((e) => {
...
export class CodeWindow extends BaseWindow implements ICodeWindow {
	...
	constructor(
		...
		let samplePeriod = parseInt(
			this.environmentMainService.args["unresponsive-sample-period"] ||
				"15000",
		);
		if (
			sampleInterval <= 0 ||
	...
	private onConfigurationUpdated(e?: IConfigurationChangeEvent): void {
		...
		if (
			...
			const inspect =
				this.configurationService.inspect<string>("http.proxy");
			let newHttpProxy =

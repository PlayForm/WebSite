	...
	implements MainThreadAuthenticationShape
{
	...
	constructor(
		...
		this._register(
			this.authenticationService.onDidUnregisterAuthenticationProvider(
				(e) => this._proxy.$onDidUnregisterAuthenticationProvider(e.id),
			),
		);
		this._register(
		...
		this._register(
			authenticationService.registerAuthenticationProviderHostDelegate({
				...
				priority:
					extHostContext.extensionHostKind ===
					ExtensionHostKind.LocalWebWorker
						? 0
						: 1,
				create: async (serverMetadata, resource) => {
					const clientId =
						this.dynamicAuthProviderStorageService.getClientId(
	...
	async $registerAuthenticationProvider(
		...
		supportedIssuers: UriComponents[] = [],
	): Promise<void> {
		if (
	...
	async $waitForUriHandler(
		expectedUri: UriComponents,
	...
	private async doGetSession(
		...
		const sessions = await this.authenticationService.getSessions(
			providerId,
			scopes,
			{ account: options.account, issuer },
			true,
		);
	...
	async $showDeviceCodeModal(
		...
		if (result) {
			...
			try {
				...
				return await this.openerService.open(

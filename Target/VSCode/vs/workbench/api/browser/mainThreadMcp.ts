...
export class MainThreadMcp extends Disposable implements MainThreadMcpShape {
	...
	private readonly _serverDefinitions = new Map<
		number,
		McpServerDefinition
	>();
			...
			const handle = this._mcpRegistry.registerCollection({
				...
				resolveServerLanch: collection.canResolveLaunch
					? async (def) => {
							...
							return r
								? McpServerLaunch.fromSerialized(r)
								: undefined;
						}
					: undefined,
	...
	async $getTokenFromServerMetadata(
		...
		let providerId =
			await this._authenticationService.getOrActivateProviderIdForIssuer(
				issuer,
			);
		if (!providerId) {
			const provider =

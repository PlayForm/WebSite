...
export class ExtHostAuthentication implements ExtHostAuthenticationShape {
	...
	async getSession(
		...
		const keys: (keyof vscode.AuthenticationGetSessionOptions)[] =
			Object.keys(
				options,
...
export class DynamicAuthProvider implements vscode.AuthenticationProvider {
	...
	constructor(
		...
		this._logger = loggerService.createLogger(_serverMetadata.issuer, {
			name: this.label,
		});
		...
		const scopedEvent = Event.chain(
			...
			($) =>
				$.filter(
					(e) =>
						e.authProviderId === this.id && e.clientId === clientId,
				).map((e) => e.tokens),
		);
	...
	async getSessions(
		...
		let sessions = this._tokenStore.sessions.filter(
			(session) => session.scopes.join(" ") === scopes.join(" "),
		);
		this._logger.info(
			`Found ${sessions.length} sessions for scopes: ${scopes.join(" ")}`,
		);
		if (sessions.length) {
			...
			const tokenMap = new Map<string, IAuthorizationToken>(
				this._tokenStore.tokens.map((token) => [
					token.access_token,
					token,
				]),
			);
	...
	async createSession(
		...
		for (let i = 0; i < this._createFlows.length; i++) {
			...
			try {
				...
				const result =
					await this._proxy.$showContinueNotification(message);
				if (!result) {
	...
	private async _createWithUrlHandler(
		...
		const callbackUri = URI.parse(
			`${this._initData.environment.appUriScheme}://dynamicauthprovider/${issuer.authority}/authorize?nonce=${nonce}`,
		);
		let state: URI;
		try {
		...
		this._logger.info(
			`Opening authorization URL for scopes: ${scopeString}`,
		);
	...
	protected async generateCodeChallenge(
		codeVerifier: string,
	): Promise<string> {
...
class TokenStore implements Disposable {
		...
		this._sessionsObservable = derivedOpts(
			{
				equalsFn: (a, b) =>
					arraysEqual(
						a,
						b,
						(a, b) => a.accessToken === b.accessToken,
					),
			},
			(reader) =>
	...
	update({
		added,
		removed,
	}: { added: IAuthorizationToken[]; removed: IAuthorizationToken[] }): void {
		this._logger.trace(

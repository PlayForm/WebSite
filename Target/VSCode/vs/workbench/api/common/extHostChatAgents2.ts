		...
		if (!this._apiObject) {
			...
			this._apiObject = Object.freeze<vscode.ChatResponseStream>({
				...
				notebookEdit(target, edits) {
					...
					const dto =
						typeConvert.ChatResponseNotebookEditPart.from(part);
					_report(dto);
					return this;
				},
				...
				prepareToolInvocation(toolName) {
					...
					const part = new extHostTypes.ChatPrepareToolInvocationPart(
						toolName,
					);
					const dto =
						typeConvert.ChatPrepareToolInvocationPart.from(part);
					_report(dto);
					return this;
				},
				push(part) {
					...
					if (
						...
						part instanceof
							extHostTypes.ChatResponseNotebookEditPart ||
						part instanceof
						...
						part instanceof
							extHostTypes.ChatResponseExtensionsPart ||
					...
					if (
						...
						part instanceof
						extHostTypes.ChatPrepareToolInvocationPart
					) {
						checkProposedApiEnabled(
							that._extension,
							"chatParticipantAdditions",
						...
						const dto =
							typeConvert.ChatPrepareToolInvocationPart.from(
								part,
							);
						_report(dto);
						return this;
	...
	private async prepareHistoryTurns(
		...
		for (const h of context.history) {
			...
			const varsWithoutTools = h.request.variables.variables
				...
				.map((v) =>
					typeConvert.ChatPromptReference.to(
						v,
...
class ExtHostChatAgent {
	...
	setChatRequestPauseState(
		pauseState: vscode.ChatParticipantPauseStateEvent,
	) {

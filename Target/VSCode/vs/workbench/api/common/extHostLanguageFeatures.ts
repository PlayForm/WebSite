			...
			if (
				...
				if (codeActionContext.only) {
					if (!toConvert.kind) {
						...
						!codeActionContext.only.contains(toConvert.kind)
					) {
						this._logService.warn(
		...
		return (
			...
			(typeof this._provider.handleDidShowCompletionItem === "function" ||
				typeof this._provider.handleDidPartiallyAcceptCompletionItem ===
					"function" ||
				typeof this._provider.handleDidRejectCompletionItem ===
					"function" ||
				...
				(item, idx) => {
					...
					return {
						...
						displayLocation:
							...
							this._isAdditionsProposedApiEnabled
								? {
										range: typeConvert.Range.from(
											item.displayLocation.range,
										),
							...
							item.warning && this._isAdditionsProposedApiEnabled
								? {
										...
										icon: item.warning.icon
											? typeConvert.IconPath.fromThemeIcon(
													item.warning.icon,
												)
											: undefined,
									}
	...
	implements extHostProtocol.ExtHostLanguageFeaturesShape
{
	...
	registerInlineCompletionsProvider(
		...
		if (eventHandle !== undefined) {
			const subscription = provider.onDidChange!((_) =>
				this._proxy.$emitInlineCompletionsChange(eventHandle),
			);
			result = Disposable.from(result, subscription);
		}

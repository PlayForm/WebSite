	...
	implements ExtHostLanguageModelToolsShape
{
	...
	async invokeTool(
		...
		try {
			...
			if (
				(toolId === InternalEditToolId ||
					toolId === ExtensionEditToolId) &&
			...
			const dto: Dto<IToolResult> =
				result instanceof SerializableObjectWithBuffers
					? result.value
					: result;
	...
	> {
			...
			progress = {
				report: (value) => {
					this._proxy.$acceptToolProgress(dto.callId, {
						message: typeConvert.MarkdownString.fromStrict(
							value.message,
						),
		...
		const extensionResult = await raceCancellation(
			Promise.resolve(
				(item.tool.invoke as any)(options, token, progress!),
			),
			token,
		);
		if (!extensionResult) {
		...
		return typeConvert.LanguageModelToolResult2.from(
			extensionResult,
			item.extension,
		);
	}
	...
	async $prepareToolInvocation(
		...
		if (
			isProposedApiEnabled(item.extension, "chatParticipantPrivate") &&
			...
			return {
				confirmationMessages: result.confirmationMessages
					? {
							title:
								typeof result.confirmationMessages.title ===
								"string"

...
export class ExtHostLanguageModels implements ExtHostLanguageModelsShape {
	...
	async getLanguageModelByIdentifier(
		...
		if (!apiObject) {
			...
			apiObject = {
				...
				capabilities: {
					supportsImageToText:
						data.metadata.capabilities?.vision ?? false,
					supportsToolCalling:
						data.metadata.capabilities?.toolCalling ?? false,
				},
	...
	private async _sendChatRequest(
		...
		try {
			...
			throw (
				extHostTypes.LanguageModelError.tryDeserialize(error) ?? error
			);
		}

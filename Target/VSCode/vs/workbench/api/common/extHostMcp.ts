	...
	implements IExtHostMpcService
{
	...
	private readonly _sseEventSources = this._register(
	...
	protected _startMcp(id: number, launch: McpServerLaunch): void {
		if (launch.type === McpServerTransportType.HTTP) {
			this._sseEventSources.set(
				id,
				new McpHTTPHandle(id, launch, this._proxy),
			);
			return;
		}
	...
	async $resolveMcpLaunch(
		...
		return resolved
			? Convert.McpServerDefinition.from(resolved)
			: undefined;
	}
	...
	public registerMcpConfigurationProvider(
		...
		const metadata =
			extension.contributes?.mcpServerDefinitionProviders?.find(
				(m) => m.id === id,
			);
		if (!metadata) {
			throw new Error(
...
class McpHTTPHandle extends Disposable {
	...
	private async _sendStreamableHttp(
		...
		const doFetch = () =>
			fetch(this._launch.uri.toString(true), {
				method: "POST",
		...
		if (res.status >= 300) {
			...
			this._proxy.$onDidChangeState(this._id, {
				...
				message:
					...
					retryWithSessionId
						? `; will retry with new session ID`
						: "",
	...
	private async _populateAuthMetadata(
		...
		if (originalResponse.headers.has("WWW-Authenticate")) {
			const authHeader =
				originalResponse.headers.get("WWW-Authenticate")!;
		...
		if (resourceMetadataChallenge) {
			const resourceMetadata = await this._getResourceMetadata(
		...
		try {
			const serverMetadataResponse =
				await this._getAuthorizationServerMetadata(
					serverMetadataUrl,
					addtionalHeaders,
				);
			const serverMetadataWithDefaults = getMetadataWithDefaultValues(
			...
			this._authMetadata = {
				server: {
					...
					scopes_supported:
						scopesSupported ??
						serverMetadataWithDefaults.scopes_supported,
				},
				resource,
			};
			return;
			...
			this._log(
				LogLevel.Warning,
				`Error populating auth metadata: ${String(e)}`,
			);
		}
		...
		defaultMetadata.scopes_supported =
			scopesSupported ?? defaultMetadata.scopes_supported ?? [];
		this._authMetadata = {
	...
	private async _getResourceMetadata(
		...
		if (isAuthorizationProtectedResourceMetadata(body)) {
			...
			throw new Error(
				`Invalid resource metadata: ${JSON.stringify(body)}`,
			);
		}
	}

	private async _getAuthorizationServerMetadata(
		...
		throw new Error(
			`Invalid authorization server metadata: ${JSON.stringify(body)}`,
		);
	}
		...
		switch (res.headers.get("Content-Type")?.toLowerCase()) {
			case "text/event-stream": {
				...
				try {
					...
					this._log(
						LogLevel.Warning,
						`Error reading SSE stream: ${String(err)}`,
					);
				}
				break;
			}
	...
	private async _doSSE(parser: SSEParser, res: Response) {
		...
		do {
			try {
				chunk = await raceCancellationError(
					reader.read(),
					this._cts.token,
				);
			} catch (err) {
		...
		if (this._authMetadata) {
			try {
				const token = await this._proxy.$getTokenFromServerMetadata(
					this._id,
					this._authMetadata.server,
					this._authMetadata.resource,
				);
				if (token) {

				...
				if (
						...
						value: coalesce(
							c.content.map((part) => {
								if (
									...
									part instanceof types.LanguageModelDataPart
								) {
									return {
										type: "data",
										value: {
											mimeType:
												part.mimeType as chatProvider.ChatImageMimeType,
					...
					if (isImageDataPart(c)) {
						const value: chatProvider.IChatImageURLPart = {
							mimeType:
								c.mimeType as chatProvider.ChatImageMimeType,
		...
		if (isUriComponents(value)) {
			...
			value = new types.ChatReferenceBinaryData(
				...
				() =>
					Promise.resolve(
						new Uint8Array(
							Object.values(variable.value as number[]),
						),
					),
...
export namespace TerminalCompletionList {
	export function from(
		...
		if (Array.isArray(completions)) {
			return {
				items: completions.map((i) =>
					TerminalCompletionItemDto.from(i),
				),
			};
		}
		return {
			items: completions.items.map((i) =>
				TerminalCompletionItemDto.from(i),
			),
	...
	export function from(
		...
		return {
			...
			cwd: resourceRequestConfig.cwd
				? URI.revive(resourceRequestConfig.cwd)
				: undefined,
		};
	}
}
...
export namespace InlineCompletionEndOfLifeReason {
	export function to<T>(
		...
		if (
			reason.kind ===
			languages.InlineCompletionEndOfLifeReasonKind.Ignored
		) {
			...
			reason.kind ===
			languages.InlineCompletionEndOfLifeReasonKind.Accepted
		) {
			return {
		...
		return new types.LanguageModelToolResult2(
			result.content.map((item) => {
				if (item.kind === "text") {
					...
					const mimeType = Object.values(
						types.ChatImageMimeType,
					).includes(item.value.mimeType as types.ChatImageMimeType)

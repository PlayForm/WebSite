				...
				case "/": {
					...
					if (this.state !== state) {
						...
						deferredPromise.error(
							new Error("State does not match."),
						);
						break;
					}
	...
	start(): Promise<void> {
		...
		this._server.on("error", (err) => {
			if ("code" in err && err.code === "EADDRINUSE") {
				this._logger.error(
					"Address in use, retrying with a different port...",
				);
				setTimeout(() => {
					this._server.close();
			...
			clearTimeout(portTimeout);
			deferredPromise.error(
				new Error(`Error listening to server: ${err}`),
			);
		});
	...
	stop(): Promise<void> {
		...
		setTimeout(() => {
			if (!deferredPromise.isResolved) {
				deferredPromise.error(
					new Error("Timeout waiting for server to close"),
				);
			}
		}, 5000);
		return deferredPromise.p;
...
export class NodeDynamicAuthProvider extends DynamicAuthProvider {
	...
	private async _createWithLoopbackServer(
		...
		try {
			...
			try {
				...
				this._logger.error(
					`Failed to receive authorization code: ${err}`,
				);
	...
	private async _createWithDeviceCode(
		...
		const deviceAuthUrl =
			this._serverMetadata.device_authorization_endpoint;
		const scopeString = scopes.join(" ");
		this._logger.info(
			`Starting device code flow for scopes: ${scopeString}`,
		);
		...
		if (!isAuthorizationDeviceResponse(deviceCodeData)) {
			this._logger.error(
				"Invalid device code response received from server",
			);
			throw new Error(
				"Invalid device code response received from server",
			);
			...
			try {
				...
				if (tokenResponse.ok) {
					...
					if (!isAuthorizationTokenResponse(tokenData)) {
						this._logger.error(
							"Invalid token response received from server",
						);
						throw new Error(
							"Invalid token response received from server",
						);

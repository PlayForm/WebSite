...
function unBatchChildren(
	result: (number | ITreeItem)[][] | undefined,
): ITreeItem[] | undefined {
			...
			.then((elements) => {
				...
				return Promise.all([
					testObject
						...
						.then((children) => {
							const actuals = unBatchChildren(children)?.map(
								(e) => e.handle,
							);
							...
							return Promise.all([
								testObject
									...
									.then((children) =>
										assert.strictEqual(
											unBatchChildren(children)?.length,
											0,
										),
									),
								testObject
		...
		return testObject
			...
			.then((elements) => {
				assert.deepStrictEqual(
					removeUnsetKeys(unBatchChildren(elements)),
					[
						{
							handle: "1/a",
							label: {
		...
		store.add(
			target.onRefresh.event(() => {
				testObject
					.$getChildren("testNodeWithIdTreeProvider")
					.then((elements) => {
						const actuals = unBatchChildren(elements)?.map(
							(e) => e.handle,
						);
						assert.deepStrictEqual(actuals, ["1/a", "1/b"]);
						return testObject
		...
		return testObject
			...
			.then((elements) => {
				...
				return elements[0]
					.slice(1)
					.map((e) =>
						loadCompleteTree(treeId, (e as ITreeItem).handle),
					);
			})
			.then(() => null);
	}

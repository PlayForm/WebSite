...
abstract class OpenChatGlobalAction extends Action2 {
	...
	override async run(
		...
		if (!switchToMode) {
			switchToMode = opts?.query.startsWith("@")
				? ChatMode.Ask
				: undefined;
		}
			...
			constructor() {
				super(ChatMode.Agent, {
					when: ContextKeyExpr.has(
						`config.${ChatConfiguration.AgentEnabled}`,
					),
					weight: KeybindingWeight.WorkbenchContrib,
		...
		class ToggleChatAction extends Action2 {
			...
			async run(accessor: ServicesAccessor) {
				...
				if (viewsService.isViewVisible(ChatViewId)) {
					...
					(
						await showCopilotView(viewsService, layoutService)
					)?.focusInput();
				}
			}
		...
		class ChatHistoryAction extends Action2 {
			...
			async run(accessor: ServicesAccessor) {
				...
				const editingSession =
					view.widget.viewModel?.model.editingSession;
				if (editingSession) {
	...
	registerAction2(
		class ChatAddAction extends Action2 {
			...
			override async run(
				...
				const widget =
					context?.widget ?? widgetService.lastFocusedWidget;
				if (!widget) {
					return;
				}
				...
				if (
					hasAgentOrCommand?.agentPart ||
					hasAgentOrCommand?.commandPart
				) {
					return;
				}
		...
		class ShowQuotaExceededDialogAction extends Action2 {
			...
			override async run(accessor: ServicesAccessor) {
				...
				const completionsQuotaExceeded =
					chatEntitlementService.quotas.completions
						?.percentRemaining === 0;

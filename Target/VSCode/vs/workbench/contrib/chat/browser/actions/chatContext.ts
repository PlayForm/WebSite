		...
		for (const entry of widget.input.selectedToolsModel.entries.get()) {
			if (entry instanceof ToolSet) {
				...
				items.push({
					...
					description:
						entry.userDescription ?? entry.modelDescription,
					asAttachment: (): IChatRequestToolEntry => {
						return {
							kind: "tool",
							id: entry.id,
							icon: ThemeIcon.isThemeIcon(entry.icon)
								? entry.icon
								: undefined,
...
class ClipboardImageContextValuePick implements IChatContextValueItem {
	...
	async isEnabled(widget: IChatWidget) {
		if (
			!widget.input.selectedLanguageModel?.metadata.capabilities?.vision
		) {
			return false;
		}
...
class ScreenshotContextValuePick implements IChatContextValueItem {
	...
	async isEnabled(widget: IChatWidget) {
		return !!widget.input.selectedLanguageModel?.metadata.capabilities
			?.vision;
	}

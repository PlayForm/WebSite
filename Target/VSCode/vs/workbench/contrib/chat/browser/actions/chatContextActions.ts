			...
			for (const match of matches) {
				if (isSearchTreeFileMatch(match)) {
					...
					if (
						...
						widget.attachmentModel.addFile(
							context.uri,
							context.range,
						);
					}
...
export class AttachContextAction extends Action2 {
			...
			if (/\.(png|jpg|jpeg|bmp|gif|tiff)$/i.test(pick.resource.path)) {
				...
				if (URI.isUri(pick.resource)) {
					...
					toAttach.push({
						...
						kind: "image",
						references: [
							{ reference: pick.resource, kind: "reference" },
						],
					});
				}
			} else {
	...
	private async _handleContextPickerItem(
		...
		if (isThenable(pickerConfig.picks)) {
			...
			const query = observableValue<string>(
				"attachContext.query",
				qp.value,
			);
			store.add(
				qp.onDidChangeValue(() => query.set(qp.value, undefined)),
			);
			...
			store.add(
				autorun((reader) => {
					...
					qp.items = ([] as QuickPickItem[]).concat(
						picks,
						extraPicks,
					);
					qp.busy = busy;

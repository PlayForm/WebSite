...
class ToggleChatModeAction extends Action2 {
	...
	private getNextMode(
		...
		const modeIndex = modes.findIndex(
			(mode) => mode.id === chatWidget.input.currentMode2.id,
		);
...
export class ChatSubmitWithCodebaseAction extends Action2 {
	...
	run(accessor: ServicesAccessor, ...args: any[]) {
		...
		const codebaseTool =
			languageModelToolsService.getToolByName("codebase");
		if (!codebaseTool) {
			return;
		}

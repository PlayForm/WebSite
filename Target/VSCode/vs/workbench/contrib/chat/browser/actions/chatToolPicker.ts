...
export async function showToolsPicker(
	...
	for (const [toolSetOrTool, picked] of toolsEntries) {
		...
		if (toolSetOrTool.source.type === "mcp") {
			...
			buttons.push({
				...
				action: () => {
					...
					editorService.openEditor({
						resource: toolSetOrTool.source.file,
					});
				},
			});
		} else {
		...
		if (toolSetOrTool instanceof ToolSet) {
			...
			bucket.children.push({
				...
				tool: toolSetOrTool,
				label:
					toolSetOrTool.toolReferenceName ??
					toolSetOrTool.displayName,
				description:
					toolSetOrTool.userDescription ??
					toolSetOrTool.modelDescription,
				indented: true,
			});
		}
	...
	const picker = store.add(
		quickPickService.createQuickPick<MyPick>({ useSeparators: true }),
	);
	...
	const _update = () => {
		...
		try {
			const items = picks.filter(
				(p): p is MyPick => p.type === "item" && Boolean(p.picked),
			);

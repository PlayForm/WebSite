 ...
 */
const MANAGE_INSTRUCTIONS_ACTION_ID =
	"workbench.action.chat.manage.instructions";
...
class ManageInstructionsFilesAction extends Action2 {
	constructor() {
		super({
			...
			title: localize2(
				"manage-instructions.capitalized.ellipses",
				"Manage Instructions Files...",
			),
 ...
 */
const getActiveInstructionsFileUri = (
	accessor: ServicesAccessor,
): URI | undefined => {
	...
	asPicker(): {
		...
		const picks = this.promptsService
			...
			.then((value) => {
				...
				value = value
					.slice(0)
					.sort((a, b) => compare(a.storage, b.storage));
					...
					result.push({
						...
						asAttachment: (): IPromptFileVariableEntry => {
							return {
								...
								id: uri.toString(),
								value: uri,
								name: this.labelService.getUriBasenameLabel(
									uri,
								),
							};
						},

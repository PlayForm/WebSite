...
import {
	...
	NEW_MODE_COMMAND_ID,
} from "../../../../promptSyntax/contributions/createPromptCommand/createPromptCommand.js";
import {
	IKeyMods,
	IQuickInputButton,
	IQuickInputService,
...
export class PromptFilePickers {
	...
	async selectPromptFile(
		...
		try {
			...
			const activeItem =
				options.resource &&
				fileOptions.find((f) =>
					extUri.isEqual(f.value, options.resource),
				);
	...
	private async _createPromptPickItems(
		...
		const newItem =
			options.optionNew !== false
				? this._getNewItem(options.type)
				: undefined;
		if (newItem) {
	...
	private _createPromptPickItem(
		...
		const description =
			storage === "user"
				...
				: this._labelService.getUriLabel(dirname(uri), {
						relative: true,
					});
			...
			try {
				...
				if (newName) {
					await this._fileService.move(
						value,
						joinPath(currentFolder, newName),
					);
				}
			} finally {

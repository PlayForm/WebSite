...
} from "./actions/promptActions/chatSaveToPromptAction.js";
import {
...
configurationRegistry.registerConfiguration({
	...
	properties: {
		...
		"chat.setup.signInWithAlternateProvider": {
			...
			type: "boolean",
			description: nls.localize(
				"chat.signInWithAlternateProvider",
				"Enable alternative sign-in provider.",
			),
			default: false,
	...
	private registerMaxRequestsSetting(): void {
		...
		const registerMaxRequestsSetting = () => {
			...
			this.experimentService
				.getTreatment<number>(treatmentId)
				.then((value) => {
					const defaultValue =
						value ??
						(this.entitlementService.entitlement ===

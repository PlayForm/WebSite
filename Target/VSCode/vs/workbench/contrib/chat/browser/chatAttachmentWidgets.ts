...
export class ImageAttachmentWidget extends AbstractChatAttachmentWidget {
	constructor(
		...
		this.telemetryService.publicLog2<
			AttachImageEvent,
			AttachImageEventClassification
		>("copilot.attachImage", {
		...
		const fullName =
			resource?.toString() || attachment.fullName || attachment.name;
		this._register(
			createImageElements(
				resource,
				attachment.name,
				fullName,
				this.element,
				attachment.value as Uint8Array,
				this.hoverService,
				ariaLabel,
...
function createImageElements(
	...
	if (resource) {
		...
		disposable.add(
			dom.addDisposableListener(element, "click", clickHandler),
		);
	}
	...
	if (
		...
		const existingPill = element.querySelector(
		...
		if (resource) {
			...
			disposable.add(
				dom.addDisposableListener(urlContainer, "click", () =>
					clickHandler(),
				),
			);
...
export class NotebookCellOutputChatAttachmentWidget extends AbstractChatAttachmentWidget {
	...
	private renderErrorOutput(
		...
		const withIcon = attachment.icon?.id
			...
			: attachmentLabel;
		const buffer =
			this.getOutputItem(resource, attachment)?.data.buffer ??
			new Uint8Array();
	...
	private getOutputItem(
		...
		const cell = notebook.cells.find(
			(c) => c.handle === parsedInfo.cellHandle,
		);
		if (!cell) {
			return undefined;
		}

...
export class ChatAttachmentsContentPart extends Disposable {
	...
	constructor(
		...
		public readonly domNode: HTMLElement | undefined = dom.$(
		...
		for (const attachment of this.variables) {
			...
			if (isAttachmentPartialOrOmitted) {
				...
				for (const selector of [
					...
					const element =
						widget.label.element.querySelector(selector);
					if (element) {
			...
			this._register(
				dom.addDisposableListener(widget.element, "dragstart", (e) =>
					this.dragStartHandler?.(attachment, e, widget.element),
				),
			);

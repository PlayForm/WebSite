			...
			renderer.render(
				markdown.content,
				{
					...
					codeBlockRendererSync: (languageId, text, raw) => {
						...
						let vulns:
							| readonly IMarkdownVulnerability[]
							| undefined;
...
class CollapsedCodeBlock extends Disposable {
	...
	render(uri: URI, isStreaming?: boolean): void {
		...
		let modifiedByResponse =
			modifiedEntry?.isCurrentlyBeingModifiedBy.get();
		const isComplete =
			!modifiedByResponse ||
		...
		this._progressStore.add(
			autorun((r) => {
				if (!editSession) {
					editSession =
						session?.editingSessionObs?.promiseResult.read(r)?.data;
					modifiedEntry = editSession?.getEntry(uri);
				}

				modifiedByResponse =
					modifiedEntry?.isCurrentlyBeingModifiedBy.read(r);

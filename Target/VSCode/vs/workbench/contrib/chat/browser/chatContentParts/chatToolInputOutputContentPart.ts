...
export type ChatCollapsibleIOPart =
	...
	| IChatCollapsibleIOResourcePart;
...
export class ChatCollapsibleInputOutputContentPart extends Disposable {
	...
	private addResourceGroup(
		...
		const entries = parts.map((part): IChatRequestVariableEntry => {
			if (
				part.kind === "data" &&
				getAttachableImageExtension(part.mimeType)
			) {
				return {
					kind: "image",
					id: generateUuid(),
					name: part.uri
						? basename(part.uri)
						: `image.${getAttachableImageExtension(part.mimeType)}`,
					value: part.value,
				...
				return {
					kind: "generic",
					id: generateUuid(),
					name: localize("chat.unknownData", "Unknown Data"),
					value: part.value,
					fullName: localize(
						"chat.unknownData.full",
						"Unknown Data with MIME type {0}",
						part.mimeType,
					),
			...
			if (part) {
				...
				this._contextMenuService.showContextMenu({
					...
					getActionsContext: () =>
						({
							parts: [part],
						}) satisfies IChatToolOutputResourceToolbarContext,
				});
			}
		};
		...
		attachments.dragStartHandler = (attachment, event, element) => {
			...
			this._instantiationService.invokeFunction((accessor) =>
				fillEditorsDragData(accessor, [part.uri!], event),
			);
		};
	...
	private addCodeBlock(
		...
		this._register(
			editorReference.object.onDidChangeContentHeight(() =>
				this._onDidChangeHeight.fire(),
			),
		);
...
class SaveResourcesAction extends Action2 {
	...
	async run(
		...
		const partBasename = (
			part: IChatCollapsibleIODataPart | IChatCollapsibleIOResourcePart,
		) =>
			part.kind === "resource" || part.uri
				? basename(part.uri!)
				: "file" + (getExtensionForMimeType(part.mimeType) || "");
		...
		if (context.parts.length === 1) {
			...
			const uri = await fileDialog.pickFileToSave(
				joinPath(defaultFilepath, partBasename(part)),
			);
			if (!uri) {
				return;
			}
			...
			await withProgress(
				uris[0],
				context.parts.map(
					(part) => () => savePart(part, true, uris[0]),
				),
			);
		}
	}

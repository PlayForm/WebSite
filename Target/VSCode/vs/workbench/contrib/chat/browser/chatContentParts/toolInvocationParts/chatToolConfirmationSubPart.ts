...
export class ToolConfirmationSubPart extends BaseChatToolInvocationSubPart {
	...
	constructor(
		...
		const { title, message, allowAutoConfirm } =
			...
			if (
				...
				const codeBlockRenderOptions: ICodeBlockRenderOptions = {
					...
					editorOptions: {
						...
						readOnly: false,
						ariaLabel:
							typeof toolInvocation.confirmationMessages.title ===
							"string"
				...
				const langId =
					this.languageService.getLanguageIdByLanguageName("json");
				const rawJsonInput = JSON.stringify(
					inputData.rawInput ?? {},
					null,
					1,
				);

	...
	private async resolveAttachmentsFromDragEvent(
		...
		if (containsDragType(e, CodeDataTransfers.NOTEBOOK_CELL_OUTPUT)) {
			...
			if (notebookOutputData) {
				return resolveNotebookOutputAttachContext(
					notebookOutputData,
					this.editorService,
				);
			}
	...
	private async downloadImageAsUint8Array(
		...
		try {
			const extractedImages =
				await this.webContentExtractorService.readImage(
					URI.parse(url),
		...
		const selection =
			this.chatWidgetService.lastFocusedWidget?.inputEditor.getSelection();

...
class ChatEditorOverlayWidget extends Disposable {
	...
	private readonly _session = observableValue<
	...
	constructor(
		...
		this._isBusy = derived((r) => {
			...
			return (
				entry?.waitsForLastEdits.read(r) ??
		...
		this._store.add(
			autorun((r) => {
				...
				if (
					!busy ||
					!value ||
					this._session.read(r)?.isGlobalEditingSession
				) {
					textProgress.innerText = "";
				} else if (value) {
					textProgress.innerText = renderStringAsPlaintext(
						value.message,
					);
				}
			}),
									...
									if (action.id === AcceptAction.ID) {
										...
										this._store.add(
											autorun((r) => {
												...
												const ctrl = that._entry
													.read(r)
													?.autoAcceptController.read(
														r,
													);
												if (ctrl) {
													const r =
	...
	constructor(
		...
		this._store.add(
			autorun((r) => {
				...
				if (
					!session.isGlobalEditingSession &&
					!inlineChatService.hideOnRequest.read(r)
				) {
				...
				if (
					...
					const changeIndex = derived((r) =>
						entry
							? entry
									.getEditorIntegration(editorPane)
									.currentIndex.read(r)
							: 0,
					);

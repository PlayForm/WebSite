	...
	implements IModifiedFileEntry
{
	...
	async acceptAgentEdits(
		...
		if (isAtomicEdits) {
				...
				if (range) {
					...
					this._editDecorations = this.modifiedModel.deltaDecorations(
						...
						[
							{
								options:
									ChatEditingModifiedDocumentEntry._atomicEditDecorationOptions,
								range,
							},
						],
					);
			...
			const newDecorations: IModelDeltaDecoration[] = [
				...
				{
					options:
						ChatEditingModifiedDocumentEntry._pendingEditDecorationOptions,
					range: new Range(
						maxLineNumber + 1,
						1,
			...
			if (maxLineNumber > 0) {
				...
				newDecorations.push({
					options:
						ChatEditingModifiedDocumentEntry._lastEditDecorationOptions,
					range: new Range(
						maxLineNumber,
						1,
						maxLineNumber,
	...
	private async _updateDiffInfo(): Promise<IDocumentDiff | undefined> {
		...
		if (
			...
			this._edit = offsetEditFromLineRangeMapping(
				this.originalModel,
				this.modifiedModel,
				diff2.changes,
			);

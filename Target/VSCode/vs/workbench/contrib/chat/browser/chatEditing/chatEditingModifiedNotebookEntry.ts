...
export class ChatEditingModifiedNotebookEntry extends AbstractChatEditingModifiedFileEntry {
	...
	constructor(
		...
		this.modifiedModel =
			this._register(modifiedResourceRef).object.notebook;
		this.originalModel =
			this._register(originalResourceRef).object.notebook;
	...
	async initializeModelsFromDiff() {
		...
		try {
			...
			const notebookDiff =
				await this.notebookEditorWorkerService.computeDiff(
					this.originalURI,
					this.modifiedURI,
				);
	...
	mirrorNotebookEdits(e: NotebookTextModelChangedEvent) {
		...
		if (
			isTransientIPyNbExtensionEvent(this.modifiedModel.notebookType, e)
		) {
			return;
		}
						...
						event.changes.forEach((change) => {
							change[2].forEach((cell, i) => {
								...
								const edits: ICellEditOperation[] = [
									{
										editType:
											CellEditType.PartialInternalMetadata,
										index,
										internalMetadata: { internalId },
									},
								];
	...
	protected override async _doAccept(
		...
		if (
			...
			(!config.autoSave ||
				!this.notebookResolver.isDirty(this.modifiedURI))
		) {
	...
	protected override async _doReject(
		tx: ITransaction | undefined,
	): Promise<void> {
		this.updateCellDiffInfo([], tx);
	...
	protected override _resetEditsState(tx: ITransaction): void {
		...
		this.cellEntryMap.forEach(
			(entry) =>
				!entry.isDisposed && entry.clearCurrentEditLineDecoration(),
		);
	}
	...
	override async acceptAgentEdits(
		...
		const cell =
			isCellUri &&
			this.modifiedModel.cells.find((cell) =>
				isEqual(cell.uri, resource),
			);
		...
		if (isLastEdits && this.newNotebookEditGenerator) {
			const notebookEdits =
				await this.newNotebookEditGenerator.generateEdits();
			this.newNotebookEditGenerator = undefined;
	...
	createModifiedCellDiffInfo(
		...
		const unchangedCell: ICellDiffInfo = {
			...
			keep: async (changes: DetailedLineRangeMapping) => {
				const [modifiedCellModel, originalCellModel] =
					await Promise.all([
		...
		const diffs = sortCellChanges(this._cellsDiffInfo.get())
			.filter(
				(d) =>
					!(
						d.type === "delete" &&
						d.originalCellIndex === deletedOriginalIndex
					),
			)
			.map((diff) => {
				if (
	...
	override equalsSnapshot(snapshot: ISnapshotEntry | undefined): boolean {
		return (
			...
			new SnapshotComparer(snapshot.original).isEqual(
				this.originalModel,
			) &&
	...
	getOrCreateModifiedTextFileEntryForCell(
		...
		disposables.add(
			autorun((r) => {
				...
				if (
					cellEntry.diffInfo.get().identical &&
					entry.type === "modified"
				) {
					entry = {
						...entry,

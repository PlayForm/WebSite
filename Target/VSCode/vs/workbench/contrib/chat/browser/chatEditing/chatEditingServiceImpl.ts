		...
		const setReadonlyFilesEnabled = () => {
			const enabled =
				...
				extensionService.extensions.some((e) =>
					e.enabledApiProposals?.includes(
						...
						Boolean(
							activeUri &&
								session.entries
									.get()
									.find((entry) =>
										isEqual(activeUri, entry.modifiedURI),
									),
						);
	...
	private readonly _currentEntries = derived<readonly IModifiedFileEntry[]>(
		...
		(r) => {
			...
			for (const session of sessions) {
				if (
					session.state.read(r) !== ChatEditingSessionState.Disposed
				) {
	...
	async resolveDiffSource(uri: URI): Promise<IResolvedMultiDiffSource> {
		...
		const thisSession = derived(this, (r) => {
			return this._editingSessionsObs
				...
				.find(
					(candidate) =>
						candidate.chatSessionId === parsed.chatSessionId,
				);
		});
		...
		(reader) => {
			...
			return entries.map((entry) => {
				if (this._showPreviousChanges) {
					const entryDiffObs =
						currentSession.getEntryDiffBetweenStops(
							entry.modifiedURI,
							undefined,
							undefined,
						);

...
function getFirstAndLastStop(
	...
	| {
			current: ResourceMap<ISnapshotEntry>;
			next: ResourceMap<ISnapshotEntry>;
	  }
	| undefined {
	...
	implements IChatEditingSession
{
	...
	public readonly canRedo = derived<boolean>((r) => {
		...
		const linearHistoryIndex = this._linearHistoryIndex.read(r);
		return (
			linearHistoryIndex < getMaxHistoryIndex(this._linearHistory.read(r))
		);
	});
	...
	private _entryDiffBetweenTextStops(
		...
		const modelRefsPromise = derived(this, (reader) => {
			...
			const promise = Promise.all(
				modelUris.map((u) =>
					this._textModelService.createModelReference(u),
				),
			).then((refs) => {
				if (store.isDisposed) {
		...
		return derived(
			(reader): ObservablePromise<IEditSessionEntryDiff> | undefined => {
				...
				if (
					entries?.before &&
					ChatEditingModifiedNotebookEntry.canHandleSnapshot(
						entries.before,
					)
				) {
					const diffService =
	...
	private _createDiffBetweenStopsObservable(
		...
		return derived((reader) => {
			return (
				diff.read(reader)?.promiseResult.read(reader)?.data || undefined
			);
		});
	}
		...
		if (next) {
			if (stopIndex === snap.stops.length - 1) {
				const postEdit = new ResourceMap(
					snap.postEdit ||
						this._createEmptySnapshot(undefined).entries,
				);
				if (
					!snap.postEdit ||

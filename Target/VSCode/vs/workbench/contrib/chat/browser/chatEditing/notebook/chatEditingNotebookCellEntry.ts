...
export class ChatEditingNotebookCellEntry extends ObservableDisposable {
	...
	acceptAgentEdits(
		...
		const newDecorations: IModelDeltaDecoration[] = [
			...
			{
				options:
					ChatEditingNotebookCellEntry._pendingEditDecorationOptions,
				range: new Range(
					maxLineNumber + 1,
					1,
		...
		if (maxLineNumber > 0) {
			...
			newDecorations.push({
				options:
					ChatEditingNotebookCellEntry._lastEditDecorationOptions,
				range: new Range(
					maxLineNumber,
					1,
					maxLineNumber,
		...
		try {
			if (
				!this._diffInfo
					...
					.changes.filter(
						(c) =>
							c.modified.equals(change.modified) &&
							c.original.equals(change.original),
					).length
			) {

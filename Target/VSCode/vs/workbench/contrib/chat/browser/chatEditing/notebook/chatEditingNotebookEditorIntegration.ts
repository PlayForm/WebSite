	...
	implements IModifiedFileEntryEditorIntegration
{
			...
			autorun((r) => {
				...
				changes.forEach((change) => {
					...
					const editor = notebookEditor.codeEditors.find(
						([vm]) =>
							vm.handle ===
							notebookModel.cells[change.modifiedCellIndex]
								.handle,
					)?.[1];
					const modifiedModel =
					...
					if (currentDiff) {
						...
						if (
							!areDocumentDiff2Equal(currentDiff.diff.get(), diff)
						) {
		...
		this._register(
			autorun((r) => {
				...
				if (changes.every((c) => c.type === "insert")) {
					...
					this.overlayToolbarDecorator?.decorate(
						changes.filter(
							(c) => c.type === "insert" || c.type === "modified",
						),
					);
				}
			}),
				...
				if (cellViewModel) {
					...
					this.revealChangeInView(
						...
						this.logService.warn(
							`Error revealing change in view: ${err}`,
						);
					});
					return true;
				}
			...
			case "modified":
				{
					...
					const isLastChangeInCell =
						currentChange.index >=
						lastChangeIndex(currentChange.change);
					const index = isLastChangeInCell
						? 0
			...
			case "delete":
				{
					...
					if (
						nextChange &&
						this._revealFirstOrLast(nextChange, true)
					) {
						return true;
					}
				}
			...
			case "delete":
				{
					...
					if (
						prevChange &&
						this._revealFirstOrLast(prevChange, false)
					) {
						return true;
					}
				}
	...
	enableAccessibleDiffView(): void {
		...
		if (cell) {
			const integration =
				this.cellEditorIntegrations.get(cell)?.integration;
	...
	async acceptNearestChange(
		...
		if (hunk) {
			...
			if ((current && !focused) || current?.change.type === "delete") {
				current.change.keep(
					current?.change.diff.get().changes[current.index],
				);
			} else if (focused) {

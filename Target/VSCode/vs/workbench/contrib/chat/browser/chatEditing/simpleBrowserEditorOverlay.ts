...
class SimpleBrowserOverlayWidget {
	...
	constructor(
		...
		private readonly _browserElementsService: IBrowserElementsService,
		@IContextMenuService
		...
		this._showStore.add(
			addDisposableListener(
				startButton.primaryButton.element,
				"click",
				async () => {
					await startElementSelection();
				},
		...
		this._showStore.add(
			addDisposableListener(configure.element, "click", () => {
				this._preferencesService.openSettings({
					jsonEditor: false,
					query: "@id:chat.sendElementsToChat.enabled,chat.sendElementsToChat.attachCSS,chat.sendElementsToChat.attachImages",
				});
			}),
		);
	}
	...
	async addElementToChat(cts: CancellationTokenSource) {
		const editorContainer = this._container.querySelector(
			".editor-container",
		) as HTMLDivElement;
		...
		const widget =
			(await showChatView(this._viewService)) ??
		...
		if (
			...
			toAttach.push({
				...
				references: fileReference
					? [{ reference: fileReference, kind: "reference" }]
					: [],
			});
	...
	getDisplayNameFromOuterHTML(outerHTML: string): string {
		...
		const idMatch = firstElementMatch[2].match(
			/\s+id\s*=\s*["']([^"']+)["']/i,
		);
		const id = idMatch ? `#${idMatch[1]}` : "";
		const classMatch = firstElementMatch[2].match(
			/\s+class\s*=\s*["']([^"']+)["']/i,
		);
		const className = classMatch
			? `.${classMatch[1].replace(/\s+/g, ".")}`
			: "";
...
class SimpleBrowserOverlayController {
	...
	constructor(
		...
		const getActiveBrowserType = () => {
			...
			const isLiveServer =
				editor?.input.editorId === "mainThreadWebview-browserPreview";
		...
		const show = async () => {
			...
			if (activeBrowserType) {
				try {
					await this._browserElementsService.startDebugSession(
						cts.token,
						activeBrowserType,
					);
				} catch (error) {

	...
	implements IHistoryNavigationWidget
{
	...
	public async getAttachedAndImplicitContext(
		...
		if (this.implicitContext?.enabled && this.implicitContext.value) {
			const implicitChatVariables =
				await this.implicitContext.toBaseEntries();
		...
		await logExecutionTime(
			"instructions tree resolve",
			this.promptInstructionsAttachmentsPart.allSettled.bind(
		...
		contextArr.push(
			...this.promptInstructionsAttachmentsPart.chatAttachments,
		);
	...
	get hasPromptFileAttachments(): boolean {
		...
		return (
			this.implicitContext.isPromptFile && this.implicitContext.enabled
		);
	}
	...
	constructor(
		...
		this._chatEditsActionsDisposables = this._register(
			new DisposableStore(),
		);
					...
					this.languageModelsService.onDidChangeLanguageModels(
						(e) => {
							...
							if (persistedModel) {
								...
								if (
									!persistedAsDefault ||
									persistedModel.metadata.isDefault
								) {
									if (
										persistedModel.metadata.isUserSelectable
									) {
										this.setCurrentLanguageModel({
	...
	public switchToNextModel(): void {
		...
		if (models.length > 0) {
			const currentIndex = models.findIndex(
				(model) =>
					model.identifier === this._currentLanguageModel?.identifier,
			);
	...
	private modelSupportedForDefaultAgent(
		...
		if (
			...
			return (
				supportsToolsAgent && !!model.metadata.capabilities?.toolCalling
			);
		}
		...
		if (
			...
			if (!hasSetDefaultMode) {
				Promise.all([
					...
					([defaultModeTreatment, defaultLanguageModelTreatment]) => {
						if (typeof defaultModeTreatment === "string") {
							...
							const defaultMode =
								validateChatMode(defaultModeTreatment);
							if (defaultMode) {
	...
	private _handleAttachedContextChange() {
		this._hasFileAttachmentContextKey.set(
			Boolean(
				this._attachmentModel.attachments.find(
					(a) => a.kind === "file",
				),
			),
		);
	...
	render(container: HTMLElement, initialValue: string, widget: IChatWidget) {
		...
		this.inputActionsToolbar
			.getElement()
			.classList.add("chat-input-toolbar");
	...
	private renderAttachedContext() {
		...
		const hasAttachments =
			...
			!this.promptInstructionsAttachmentsPart.empty;
		dom.setVisibility(
			Boolean(
				hasAttachments ||
		...
		if (URI.isUri(implicitUri)) {
			const currentlyAttached = attachments.some(
				([, attachment]) =>
					...
					isEqual(attachment.value, implicitUri),
			);
...
const historyKeyFn = (entry: IChatHistoryEntry) =>
	JSON.stringify({
		...entry,
		state: { ...entry.state, chatMode: undefined },
	});

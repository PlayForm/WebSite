	...
	implements ITreeRenderer<ChatTreeItem, FuzzyScore, IChatListItemTemplate>
{
	...
	renderTemplate(container: HTMLElement): IChatListItemTemplate {
		...
		templateDisposables.add(
			dom.addDisposableListener(rowContainer, "mouseenter", () => {
		...
		templateDisposables.add(
			dom.addDisposableListener(rowContainer, "mouseleave", () => {
				if (isRequestVM(template.currentElement)) {
	...
	private updateItemHeightOnRender(
		element: ChatTreeItem,
		templateData: IChatListItemTemplate,
	) {
	...
	private renderChatContentDiff(
		...
		partsToRender.forEach((partToRender, contentIndex) => {
			...
			const alreadyRenderedPart =
				templateData.renderedParts?.[contentIndex];
	...
	private getNextProgressiveRenderContent(element: IChatResponseViewModel): {
		...
		if (this.shouldShowWorkingProgress(element, partsToRender)) {
			...
			partsToRender.push({
				kind: "working",
				isPaused,
				setPaused: (p) => element.model.setPaused(p),
			});
		}
	...
	private shouldShowWorkingProgress(
		...
		if (
			...
			((lastPart.kind === "textEditGroup" ||
				...
				!partsToRender.some(
					(part) =>
						part.kind === "toolInvocation" && !part.isComplete,
				)) ||
	...
	private renderChatContentPart(
		...
		try {
			if (content.kind === "treeData") {
				...
				return this.renderToolInvocation(
					content,
					context,
					templateData,
				);
			} else if (content.kind === "extensions") {
				return this.renderExtensionsContent(
					content,
					context,
					templateData,
				);
				...
				return this.renderChatErrorDetails(
					context,
					content,
					templateData,
				);
	...
	private renderChatErrorDetails(
		...
		const isLast =
			context.elementIndex === this.delegate.getListLength() - 1;
		if (content.errorDetails.isQuotaExceeded) {
			...
			const level =
				content.errorDetails.level ??
				(content.errorDetails.responseIsFiltered
					? ChatErrorLevel.Info
					: ChatErrorLevel.Error);
			return this.instantiationService.createInstance(
				ChatErrorContentPart,
			...
			toDisposable(() => {
				...
				if (codeBlocksByResponseId) {
					...
					part.codeblocks?.forEach((info, i) => {
						...
						if (
							codeblock?.ownerMarkdownPartId ===
							part.codeblocksPartId
						) {
			...
			part.addDisposable!(
				thenIfNotDisposed(info.uriPromise, (uri) => {
					...
					part.addDisposable!(
						toDisposable(() => {
							const codeblock =
								this.codeBlocksByEditorUri.get(uri);
							if (
								codeblock?.ownerMarkdownPartId ===
								part.codeblocksPartId
							) {

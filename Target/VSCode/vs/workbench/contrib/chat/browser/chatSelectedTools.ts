...
export class ChatSelectedTools extends Disposable {
	...
	constructor(
		...
		this._allTools = observableFromEvent(
			_toolsService.onDidChangeTools,
			...
			autorun((r) => {
				...
				transaction((tx) => {
					for (const tool of tools) {
						const enabled =
							!disabledData || !disabledData.toolIds.has(tool.id);
					...
					for (const toolSet of toolSets) {
						const enabled =
							!disabledData ||
							!disabledData.toolSetIds.has(toolSet.id);

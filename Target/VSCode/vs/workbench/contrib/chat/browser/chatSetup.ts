...
class ChatSetup {
	...
	private async showDialog(): Promise<ChatSetupStrategy> {
		let dialogVariant = this.configurationService.getValue<
			| "default"
			| "modern"
			| "brand-gh"
			| "brand-vsc"
		...
		if (
			this.context.state.entitlement !== ChatEntitlement.Unknown &&
			(dialogVariant === "input-email" ||
				dialogVariant === "account-create")
		) {
			dialogVariant =
				this.productService.quality !== "stable" ? "modern" : "default"; // fallback to modern/default for users that are signed in already
		}
	...
	private getInputs(
		variant: "input-email" | unknown,
	): IDialogInputOptions[] | undefined {
		...
		return [
			{
				placeholder: localize(
					"emailOrUserIdPlaceholder",
					"Enter your email or {0} username",
					defaultChat.providerName,
				),
			},
		];
	...
	> {
		...
		if (this.context.state.entitlement === ChatEntitlement.Unknown) {
			...
			switch (variant) {
				case "input-email":
					buttons = coalesce([
						[
							localize("continueWithEmailOrUserId", "Continue"),
							ChatSetupStrategy.SetupWithoutEnterpriseProvider,
							undefined,
						],
						[
							localize("createAccount", "Create a New Account"),
							ChatSetupStrategy.SetupWithAccountCreate,
							{
								styleButton: (button) => {
									button.element.classList.add("link-button");

									const separator =
										button.element.parentElement?.appendChild(
											$(".buttons-separator"),
										);
		...
		buttons.push([
			...
			{
				styleButton: (button) =>
					button.element.classList.add("link-button", "skip-button"),
			},
		]);
				...
				createWorkbenchDialogOptions(
					{
						...
						renderBody: (body) =>
							body.appendChild(
								this.createLegacyDialog(disposables),
							),

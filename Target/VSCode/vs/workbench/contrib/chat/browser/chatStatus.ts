...
const defaultChat = {
	...
	nextEditSuggestionsSetting:
		product.defaultChatAgent?.nextEditSuggestionsSetting ?? "",
	...
	private getEntryProps(): IStatusbarEntry {
		...
		if (!isNewUser(this.chatEntitlementService)) {
			...
			const completionsQuotaExceeded =
				this.chatEntitlementService.quotas.completions
					?.percentRemaining === 0;
...
function isNewUser(chatEntitlementService: IChatEntitlementService): boolean {
	return (
		!chatEntitlementService.sentiment.installed || // copilot not installed
		chatEntitlementService.entitlement === ChatEntitlement.Available
	); // not yet signed up to copilot
}
...
class ChatStatusDashboard extends Disposable {
	...
	private readonly quotaPercentageFormatter = safeIntl.NumberFormat(
		undefined,
		{ maximumFractionDigits: 1, minimumFractionDigits: 0 },
	);
	private readonly quotaOverageFormatter = safeIntl.NumberFormat(undefined, {
	...
	show(token: CancellationToken): HTMLElement {
		...
		if (chatQuota || completionsQuota || premiumChatQuota) {
			...
			const completionsQuotaIndicator =
				completionsQuota &&
				(completionsQuota.total > 0 || completionsQuota.unlimited)
					...
					: undefined;
			const chatQuotaIndicator =
				chatQuota && (chatQuota.total > 0 || chatQuota.unlimited)
					? this.createQuotaIndicator(
							this.element,
							disposables,
							chatQuota,
					...
					: undefined;
			const premiumChatQuotaIndicator =
				premiumChatQuota &&
				(premiumChatQuota.total > 0 || premiumChatQuota.unlimited)
					? this.createQuotaIndicator(
							this.element,
							disposables,
							premiumChatQuota,
				...
				this.element.appendChild(
					$(
						...
						localize(
							"limitQuota",
							"Allowance resets {0}.",
							this.dateFormatter.value.format(
								new Date(resetDate),
							),
						),
					),
			...
			if (
				this.chatEntitlementService.entitlement ===
					ChatEntitlement.Free &&
				(Number(chatQuota?.percentRemaining) <= 25 ||
					Number(completionsQuota?.percentRemaining) <= 25)
			) {
				const upgradeProButton = disposables.add(
		...
		// Settings
		{
			...
			addSeparator(
				...
				chatSentiment.installed && !chatSentiment.disabled
					? toAction({
		...
		// New to Copilot / Signed out
		{
			...
			if (newUser || signedOut || disabled) {
				...
				this.element.appendChild(
					$("div.description", undefined, descriptionText),
				);
	...
	private renderHeader(
		...
		const header = container.appendChild(
			$("div.header", undefined, label ?? ""),
		);
	...
	private renderContributedChatStatusItem(item: ChatStatusEntry): {
		element: HTMLElement;
		disposables: DisposableStore;
	} {
		...
		this.renderHeader(
			...
			headerLink
				? toAction({
						...
						run: () =>
							this.runCommandAndClose(() =>
								this.openerService.open(URI.parse(headerLink)),
							),
					})
				: undefined,
		);
	...
	private createQuotaIndicator(
		...
		label: string,
		supportsOverage: boolean,
	): (quota: IQuotaSnapshot) => void {
		...
		if (
			...
			const manageOverageButton = disposables.add(
				new Button(quotaIndicator, {
					...defaultButtonStyles,
					secondary: true,
				}),
			);
			...
			disposables.add(
				manageOverageButton.onDidClick(() =>
					this.runCommandAndClose(() =>
						this.openerService.open(
							URI.parse(defaultChat.manageOverageUrl),
						),
					),
				),
			);
		...
		const update = (quota: IQuotaSnapshot) => {
			...
			if (quota.unlimited) {
				...
				quotaValue.textContent = localize(
					"quotaDisplay",
					"{0}%",
					this.quotaPercentageFormatter.value.format(usedPercentage),
				);
			}
	...
	private createSetting(
		...
		disposables.add(
			this.configurationService.onDidChangeConfiguration((e) => {
				if (
					settingIdsToReEvaluate.some((id) =>

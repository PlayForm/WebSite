	...
	constructor(
		...
		if (
			...
			if (lastEditsState.sessionId) {
				...
				if (
					!this.chatService.isPersistedSessionEmpty(
					...
					this.viewState.inputState = {
						...
						chatMode:
							lastEditsState.inputState?.chatMode ??
							ChatMode.Edit,
					};
			...
			this.chatAgentService.onDidChangeAgents(() => {
				if (
					...
					if (!this._widget?.viewModel && !this._restoringSession) {
						const info =
							this.getTransferredOrPersistedSessionInfo();
	...
	override shouldShowWelcome(): boolean {
		...
		const hasCoreAgent = this.chatAgentService
			...
			.some(
				(agent) =>
					agent.isCore &&
					agent.locations.includes(this.chatOptions.location),
			);

...
import {
	...
	toChatVariable,
} from "./chatAttachmentModel/chatPromptAttachmentsCollection.js";
...
export class ChatWidget extends Disposable implements IChatWidget {
	...
	constructor(
		...
		this.isRequestPaused =
			ChatContextKeys.isRequestPaused.bindTo(contextKeyService);
		this.canRequestBePaused =
			ChatContextKeys.canRequestBePaused.bindTo(contextKeyService);
		...
		this._register(
			bindContextKey(
				...
				(r) => {
					return (
						this._editingSession.read(r)?.canUndo.read(r) || false
					);
				},
			),
		);
		this._register(
			bindContextKey(
				...
				(r) => {
					return (
						this._editingSession.read(r)?.canRedo.read(r) || false
					);
				},
			),
		);
		...
		this._register(
			autorun((r) => {
				...
				const session = sessions.find(
					(candidate) =>
						candidate.chatSessionId === viewModel?.sessionId,
				);
	...
	private renderWelcomeViewContentIfNeeded() {
		...
		if (!numItems) {
			...
			const additionalMessage =
				defaultAgent?.metadata.additionalWelcomeMessage;
			this.welcomePart.value = this.instantiationService.createInstance(
				...
				{ ...welcomeContent, tips, additionalMessage },
				{
					...
					isWidgetAgentWelcomeViewContent:
						this.input?.currentMode === ChatMode.Agent,
				},
			);
			dom.append(
				this.welcomeMessageContainer,
	...
	private async renderFollowups(): Promise<void> {
		if (
			...
			this.inputPart.currentMode === ChatMode.Ask
		) {
			this.inputPart.renderFollowups(
	...
	private createInput(
		...
		this.inputPart = this._register(
			this.instantiationService.createInstance(
				...
				this.location,
				{
					...
					renderWorkingSet:
						this.viewOptions.enableWorkingSet === "explicit",
		...
		this._register(
			this.inputPart.attachmentModel.onDidChange(() => {
		...
		const enabledToolSetsAndTools =
			this.input.selectedToolsModel.entries.map((value) => {
		...
		this._register(
			autorun((r) => {
				...
				this.inputPart.attachmentModel.updateContext(
					disabledTools,
					Iterable.empty(),
				);
				this.refreshParsedInput();
	...
	setModel(model: IChatModel, viewState: IChatViewState): void {
		...
		this.viewModelDisposables.add(
			Event.runAndSubscribe(
				Event.accumulate(this.viewModel.onDidChange, 0),
				(events) => {
					if (!this.viewModel) {
						return;
					}
					...
					this.requestInProgress.set(
						this.viewModel.requestInProgress,
					);
					this.isRequestPaused.set(
						this.viewModel.requestPausibility ===
	...
	private async _applyPromptFileIfSet(requestInput: {
		input: string;
		attachedContext: IChatRequestVariableEntry[];
		...
		if (agentSlashPromptPart) {
			metadata = await this.promptsService.resolvePromptSlashCommand(
				agentSlashPromptPart.slashPromptCommand,
			);
			if (metadata) {
				const uri = metadata.uri;
				if (
					!requestInput.attachedContext.some(
						(variable) =>
							isPromptFileChatVariable(variable) &&
							isEqual(
								IChatRequestVariableEntry.toUri(variable),
								uri,
							),
					)
				) {
					...
					const variable = toChatVariable(
						{ uri: metadata.uri, isPromptFile: true },
						true,
					);
				...
				requestInput.input = this.parsedInput.parts
					.filter(
						(part) => !(part instanceof ChatRequestSlashPromptPart),
					)
					.map((part) => part.text)
					.join("")
					.trim();
			...
			const uri = this._findPromptFileInContext(
				requestInput.attachedContext,
			);
			if (uri) {
	...
	private async _acceptInput(
		...
		if (this.viewModel) {
			...
			const requestInputs = {
				...
				attachedContext:
					await this.inputPart.getAttachedAndImplicitContext(
						this.viewModel.sessionId,
					),
			};
			...
			if (instructionsEnabled) {
				...
				await this.autoAttachInstructions(
			...
			if (
				...
				this.input.currentMode === ChatMode.Edit &&
	...
	getUserSelectedTools(): Record<string, boolean> | undefined {
		if (this.input.currentMode2.customTools) {
			return this.toolsService.toEnablementMap(
				this.input.currentMode2.customTools,
			);
			...
			for (const [
				tool,
				enablement,
			] of this.inputPart.selectedToolsModel.asEnablementMap()) {
	...
	layout(height: number, width: number): void {
		...
		this.inputPart.layout(
			this._dynamicMessageLayoutData?.enabled
	...
	private async _applyPromptMetadata(
		...
		this.inputPart.selectedToolsModel.enable(
			enabledToolSets,
			enabledTools,
			true,
		);
	...
	private async autoAttachInstructions(
		...
		const automaticInstructions =
			await this.promptsService.findInstructionFilesFor(variableUris);
		...
		this.inputPart.attachmentModel.promptInstructions.add(

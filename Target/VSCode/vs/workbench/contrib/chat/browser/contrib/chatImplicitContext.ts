	...
	constructor(
				...
				if (notebookEditor) {
					...
					activeEditorDisposables.add(
						notebookEditor.onDidChangeActiveCell(() => {
							...
							if (
								codeEditor &&
								codeEditor.getModel()?.uri.scheme ===
		...
		this._register(
			this.chatService.onDidSubmitRequest(({ chatSessionId }) => {
				...
				if (
					this._implicitContextEnablement[widget.location] ===
						"first" &&
	...
	private async updateImplicitContext(
		...
		for (const widget of widgets) {
			...
			if (setting === "first" && !isFirstInteraction) {
				...
				setting === "always" ||
				(setting === "first" && isFirstInteraction)
			) {
				widget.input.implicitContext.setValue(
					newValue,
					isSelection,
	...
	implements IChatRequestImplicitVariableEntry
{
	 ...
	 */
	private addPrompt(value: URI | Location): void {

			...
			this.languageFeaturesService.completionProvider.register(
				...
				},
				{
					...
					provideCompletionItems: async (
						...
						return {
							suggestions: promptCommands.map(
								(c, i): CompletionItem => {
			...
			this.languageFeaturesService.completionProvider.register(
				...
				},
				{
					...
					provideCompletionItems: async (
						...
						return {
							suggestions: mcpService.servers
								.get()
								.flatMap((server) =>
									server.prompts
										.get()
										.map((prompt): CompletionItem => {
			...
			const attachBlob = async (
				...
				if (uriStr) {
					for (const uri of [
						...
						try {
							validURI ||= (await fileService.exists(uri))
								? uri
								: undefined;
						} catch {
							// ignored
	...
	constructor(
		...
		this.registerVariableCompletions(
			...
			async ({ widget, range }, token) => {
				...
				await this.addFileAndFolderEntries(
					widget,
					result,
					range,
					token,
		...
		this.registerVariableCompletions(
			...
			({ widget, range }, token) => {
				...
				const basename =
					this.labelService.getUriBasenameLabel(currentResource);
				...
				const description =
					this.labelService.getUriLabel(currentResource, {
						relative: true,
					}) + fullRangeText;
						...
						arguments: [
							new ReferenceArgument(widget, {
								...
								range: {
									...
									endColumn:
										range.replace.startColumn + text.length,
								},
								data: {
		...
		if (pattern) {
			...
			for (const workspace of workspaces) {
				...
				for (const file of files) {
					if (!seen.has(file)) {
						result.suggestions.push(
							makeCompletionItem(file, FileKind.FILE),
						);
						seen.add(file);
					}
				}
			...
			this.languageFeaturesService.completionProvider.register(
				...
				},
				{
					...
					provideCompletionItems: async (
						...
						const iter =
							widget.input.selectedToolsModel.entries.get();

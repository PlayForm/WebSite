	...
	implements ILanguageModelToolsService
{
		...
		try {
			...
			if (tool.data.alwaysDisplayInputOutput) {
				toolResult.toolResultDetails = {
					...
					output: [{ type: "text", value: String(err) }],
					isError: true,
				};
			}
	...
	private async prepareToolInvocation(
		...
		if (prepared?.confirmationMessages) {
			if (
				...
				typeof prepared.confirmationMessages.allowAutoConfirm !==
					"boolean"
			) {
			...
			if (
				...
				tool.data.alwaysDisplayInputOutput
			) {
				prepared.toolSpecificData = {
					kind: "input",
	...
	private playAccessibilitySignal(
		...
		const setting: { sound: "auto" | "on" | "off" } =
	...
	private toolResultToIO(
		...
		return toolResult.content.map((part) => {
			if (part.kind === "text") {
				...
				return {
					type: "data",
					value64: encodeBase64(part.value.data),

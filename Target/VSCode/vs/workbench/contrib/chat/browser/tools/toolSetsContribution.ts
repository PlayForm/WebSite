	...
	static from(data: unknown) {
		...
		for (const [name, value] of Object.entries(data as RawToolSetsShape)) {
			...
			if (isFalsyOrEmpty(value.tools)) {
				throw new Error(
					`Tool set '${name}' cannot have an empty tools array`,
				);
			}
	...
	constructor(
		...
		this._store.add(
			autorun((r) => {
				...
				const toolSets =
					this._languageModelToolsService.toolSets.read(r);
				...
				for (const tool of tools) {
					if (tool.canBeReferencedInPrompt) {
						data.push({
							...
							sourceLabel: ToolDataSource.classify(tool.source)
								.label,
							sourceOrdinal: ToolDataSource.classify(tool.source)
								.ordinal,
							description:
				...
				for (const toolSet of toolSets) {
					data.push({
						...
						sourceLabel: ToolDataSource.classify(toolSet.source)
							.label,
						sourceOrdinal: ToolDataSource.classify(toolSet.source)
							.ordinal,
				...
				for (const entry of entries) {
					...
					for (const [name, value] of data.entries) {
						...
						value.tools.forEach((name) => {
							const tool =
								this._languageModelToolsService.getToolByName(
									name,
								);
							if (tool) {
								tools.push(tool);
							...
							const toolSet =
								this._languageModelToolsService.getToolSetByName(
									name,
								);
							if (toolSet) {
						...
						const toolset =
							this._languageModelToolsService.createToolSet(
								{
									type: "user",
									file: entry.resource,
						...
						transaction((tx) => {
							store.add(toolset);
							tools.forEach((tool) =>
								store.add(toolset.addTool(tool, tx)),
							);
...
export class ConfigureToolSets extends Action2 {
	...
	override async run(accessor: ServicesAccessor): Promise<void> {
		...
		const picks: (
			| (IQuickPickItem & { toolset?: ToolSet })
			| IQuickPickSeparator
		)[] = [];

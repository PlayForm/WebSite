...
export interface IGenericChatRequestVariableEntry
	extends IBaseChatRequestVariableEntry {
	kind: "generic";
}

export interface IChatRequestDirectoryEntry
	extends IBaseChatRequestVariableEntry {
	kind: "directory";
}
...
export interface IChatRequestToolSetEntry
	extends IBaseChatRequestVariableEntry {
...
export interface IPromptFileVariableEntry
	extends IBaseChatRequestVariableEntry {
...
class ResponseView extends AbstractResponse {
	constructor(
		...
		super(
			idx === -1
				? _response.value.slice()
				: _response.value.slice(0, idx),
		);
	}
}
	...
	implements IChatResponseModel
{
	...
	public set shouldBeRemovedOnSend(disablement:
...
export class ChatModel extends Disposable implements IChatModel {
	...
	constructor(
		...
		this._creationDate =
			(isValid && initialData.creationDate) || Date.now();
	...
	toggleLastRequestPaused(isPaused?: boolean) {
		if (
			...
			const pausedValue =
				isPaused ?? !this.lastRequest.response.isPaused.get();
	...
	addRequest(
		...
		const request = new ChatRequestModel({
			...
			modelId,
			editedFileEvents: editedFileEvents.length
				? editedFileEvents
				: undefined,
		});
		request.response = new ChatResponseModel({

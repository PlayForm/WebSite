...
export class ChatService extends Disposable implements IChatService {
	...
	private saveState(): void {
		...
		if (this.useFileStorage) {
			...
			if (this.isEmptyWindow) {
				...
				let allSessions: (ChatModel | ISerializableChatData)[] =
					liveChats;
					...
					const prepareChatAgentRequest = (
						...
						return {
							...
							toolSelectionIsExclusive:
								options?.toolSelectionIsExclusive,
	...
	private resolveModelId(
		...
		return (
			userSelectedModelId &&
			this.languageModelsService.lookupLanguageModel(userSelectedModelId)
				?.id
		);
	}
	...
	private async checkAgentAllowed(agent: IChatAgentData): Promise<void> {
		if (agent.modes.includes(ChatMode.Agent)) {
			const enabled =
				await this.experimentService.getTreatment<boolean>(
					"chatAgentEnabled",
				);
			if (enabled === false) {

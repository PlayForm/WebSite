...
import {
	...
	type TDecorationClass,
	type TChangedDecorator,
} from "./decorations/utils/index.js";
...
registerThemingParticipant((_theme, collector) => {
	for (const Decoration of SUPPORTED_DECORATIONS) {
		for (const [className, styles] of Object.entries(
			Decoration.cssStyles,
		)) {
			collector.addRule(
				`.monaco-editor ${className} { ${styles.join(" ")} }`,
			);
		}
	}
});

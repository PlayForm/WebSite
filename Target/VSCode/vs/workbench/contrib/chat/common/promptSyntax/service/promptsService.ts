...
import {
	logTime,
	TLogFunction,
} from "../../../../../../base/common/decorators/logTime.js";
...
export class PromptsService extends Disposable implements IPromptsService {
		...
		this.cache = this._register(
			new ObjectCache((model) => {
				...
				const parser: TextModelPromptParser = instantiationService
					.createInstance(TextModelPromptParser, model, {
						seenReferences: [],
					})
					.start();
	...
	public get onDidChangeCustomChatModes(): Event<void> {
		if (!this.onDidChangeCustomChatModesEvent) {
			this.onDidChangeCustomChatModesEvent =
				this.fileLocator.getFilesUpdatedEvent(PromptsType.mode);
		}
	...
	public async listPromptFiles(
		...
		const prompts = await Promise.all([
			this.fileLocator
				.listFiles(type, "user", token)
				.then(withType("user", type)),
			this.fileLocator
				.listFiles(type, "local", token)
	...
	public asPromptSlashCommand(
		command: string,
	): IChatPromptSlashCommand | undefined {
		if (command.match(/^[\w_\-\.]+$/)) {
			return {
				command,
				detail: localize(
	...
	private async getPromptPath(
		...
		const result = files.find(
			(file) => getPromptCommandName(file.uri.path) === command,
		);
		if (result) {
			return result.uri;
		}
		...
		return promptFiles.map((promptPath) => {
			...
			return {
				...
				detail: localize(
					"prompt.file.detail",
					"Prompt file: {0}",
					this.labelService.getUriLabel(promptPath.uri, {
						relative: true,
					}),
				),
				promptPath,
	...
	public async getCustomChatModes(): Promise<readonly ICustomChatMode[]> {
		const modeFiles = (
			await this.listPromptFiles(PromptsType.mode, CancellationToken.None)
			...
			modeFiles.map(async (uri): Promise<ICustomChatMode> => {
				...
				try {
					...
					parser = this.instantiationService
						.createInstance(PromptParser, uri, {

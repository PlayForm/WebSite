...
export class PromptFilesLocator extends Disposable {
	...
	public getFilesUpdatedEvent(type: PromptsType): Event<void> {
		...
		let parentFolders = this.getLocalParentFolders(type).map(
			(folder) => folder.parent,
		);
		this._register(
			this.configService.onDidChangeConfiguration((e) => {
				if (e.affectsConfiguration(key)) {
					parentFolders = this.getLocalParentFolders(type).map(
						(folder) => folder.parent,
					);
					eventEmitter.fire();
				}
		...
		this._register(
			this.fileService.onDidFilesChange((e) => {
				if (
					e.affects(this.userDataService.currentProfile.promptsHome)
				) {
					eventEmitter.fire();
					return;
				}
	...
	private async listFilesInLocal(
		...
		for (const { parent, filePattern } of this.getLocalParentFolders(
			type,
		)) {
			const files =
				filePattern === undefined
					...
					: await this.searchFilesInLocation(
							parent,
							filePattern,
							token,
						);

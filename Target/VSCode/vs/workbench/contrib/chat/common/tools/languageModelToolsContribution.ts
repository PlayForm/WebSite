			...
			items: {
				...
				properties: {
					name: {
						description: localize(
							"toolSetName",
							"A name for this tool set. Used as reference and should not contain white.",
						),
						type: "string",
						pattern: "^[\\w-]+$",
					},
					...
					tools: {
						description: localize(
							"toolSetTools",
							"An array of tool or tool set names that are part of this set.",
						),
						type: "array",
						items: {
							type: "string",
			...
			for (const extension of delta.added) {
				for (const rawTool of extension.value) {
					...
					if (
						rawTool.tags?.some(
							(tag) =>
								tag.startsWith("copilot_") ||
								tag.startsWith("vscode_"),
						) &&
						!isProposedApiEnabled(
							extension.description,
							"chatParticipantPrivate",
					...
					const isBuiltinTool = productService.defaultChatAgent
						?.chatExtensionId
			...
			for (const extension of delta.added) {
				...
				for (const toolSet of extension.value) {
					...
					for (const toolName of toolSet.tools) {
						...
						const toolSetObj =
							languageModelToolsService.getToolSetByName(
								toolName,
							);
						if (toolSetObj) {

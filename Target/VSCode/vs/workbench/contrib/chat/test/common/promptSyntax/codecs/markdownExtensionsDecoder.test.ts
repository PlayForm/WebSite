...
class TestEndOfLine extends Text<(NewLine | CarriageReturn)[]> {
	...
	public static create(
		...
		tokens.push(
			new NewLine(
				new Range(lineNumber, startColumn, lineNumber, startColumn + 1),
			),
		);
	...
	suite("• Front Matter header", () => {
		suite("• successful cases", () => {
			test("• produces expected tokens", async () => {
				...
				const startMarker = TestFrontMatterMarker.create(
					markerLength,
					1,
					newLine,
				);
				const endMarker = TestFrontMatterMarker.create(
					markerLength,
					4,
					newLine,
				);
			...
			test("• can contain dashes in the header contents", async () => {
				...
				const promptContents = [
					// start marker
					new Array(markerLength)
						.fill("-")
						.join(""),
					// contents
					"variables: ",
					new Array(dashesLength)
						.fill("-")
						.join(""), // dashes inside the contents
					"  - name: value\t",
					// end marker
					new Array(markerLength)
						.fill("-")
				...
				await test.run(promptContents.join(newLine), [
					...
					new FrontMatterHeader(
						...
						new Text([
							...
							...TestFrontMatterMarker.create(
								dashesLength,
								3,
								newLine,
							).dashTokens,
							...TestEndOfLine.create(
								newLine,
								3,
								dashesLength + 1,
							).children,

...
} from "../common/installExtensionsTool.js";
	...
	implements IWorkbenchContribution
{
	...
	private registerGlobalActions(): void {
		...
		this.registerExtensionAction({
			...
			run: () =>
				this.extensionsWorkbenchService.openSearch("@installed "),
		});
			...
			run: async (accessor: ServicesAccessor) => {
				...
				if (result) {
					...
					trustedPublishers.filter(
						(publisher) =>
							!result.some((r) => r.id === publisher.publisher),
					);
...
class TrustedPublishersInitializer implements IWorkbenchContribution {
	constructor(
		...
		const trustedPublishersInitStatusKey =
				...
				extensionManagementService
					...
					.then(async (extensions) => {
						...
						for (const extension of extensions) {
							...
							const publisher =
								extension.manifest.publisher.toLowerCase();
							if (
	...
	implements IWorkbenchContribution
{
	...
	constructor(
		...
		const installExtensionsTool = instantiationService.createInstance(
			InstallExtensionsTool,
		);
		this._register(
			toolsService.registerToolData(InstallExtensionsToolData),
		);
		this._register(
			toolsService.registerToolImplementation(
				InstallExtensionsToolData.id,
				installExtensionsTool,
			),
		);
	}

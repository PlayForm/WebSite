...
export class SearchExtensionsTool implements IToolImpl {
	...
	async invoke(
		...
		const queryAndAddExtensions = async (text: string) => {
			const extensions =
				await this.extensionWorkbenchService.queryGallery(
					{
						text,
						pageSize: 10,
		...
		if (params.ids?.length) {
			const extensions =
				await this.extensionWorkbenchService.getExtensions(
				...
				output: [
					{
						...
						value: JSON.stringify(
							result.map((extension) => extension.id),
						),
					},
				],
			},

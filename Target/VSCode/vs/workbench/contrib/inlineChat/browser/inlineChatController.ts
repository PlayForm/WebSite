...
export class InlineChatController implements IEditorContribution {
	...
	private readonly _delegate: IObservable<
		InlineChatController1 | InlineChatController2
	>;
...
export class InlineChatController1 implements IEditorContribution {
	...
	constructor(
		...
		private readonly _webContentExtractorService: ISharedWebContentExtractorService,
	...
	async run(
		options: InlineChatRunOptions | undefined = {},
	): Promise<boolean> {
		...
		this._sessionStore.add(
			this._editor.onDidChangeModelContent((e) => {
				if (
					...
					this._ui.value.widget.hasFocus()
				) {
					return;
				}
		...
		if (options.attachments) {
			await Promise.all(
				options.attachments.map(async (attachment) => {
					await this._ui.value.widget.chatWidget.attachmentModel.addFile(
						attachment,
					);
				}),
			);
	...
	async createImageAttachment(
		...
		if (attachment.scheme === Schemas.file) {
			...
			attachment.scheme === Schemas.http ||
			attachment.scheme === Schemas.https
		) {
			const extractedImages =
				await this._webContentExtractorService.readImage(
					attachment,
					CancellationToken.None,
				);
			if (extractedImages) {
...
export class InlineChatController2 implements IEditorContribution {
	...
	static get(editor: ICodeEditor): InlineChatController2 | undefined {
		return (
			editor.getContribution<InlineChatController2>(
	...
	constructor(
		...
		@IInlineChatSessionService
		private readonly _inlineChatSessions: IInlineChatSessionService,
		...
		const visibleSessionObs = observableValue<
			IInlineChatSession2 | undefined
		>(this, undefined);
		...
		this._store.add(
			autorun((r) => {
				...
				if (!session) {
					...
					const requestCount = observableFromEvent(
						this,
						session.chatModel.onDidChange,
						() => session.chatModel.getRequests().length,
					).read(r);
		...
		this._store.add(
			autorun((r) => {
				...
				const pane = this._editorService.visibleEditorPanes.find(
					(candidate) => candidate.getControl() === this._editor,
				);
				if (pane && entry) {
	...
	async run(arg?: InlineChatRunOptions): Promise<boolean> {
		...
		const uri = this._editor.getModel().uri;
		const session =
			this._inlineChatSessions.getSession2(uri) ??
			(await this._inlineChatSessions.createSession2(
				this._editor,
				uri,
				CancellationToken.None,
			));
			...
			if (arg.attachments) {
				await Promise.all(
					arg.attachments.map(async (attachment) => {
						await this._zone.value.widget.chatWidget.attachmentModel.addFile(
							attachment,
						);
					}),
				);

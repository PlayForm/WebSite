			...
			autorun((reader) => {
				const servers = groupBy(
					mcpService.servers
						...
						.sort(
							(a, b) =>
								(a.collection.presentation?.order || 0) -
								(b.collection.presentation?.order || 0),
						),
					(s) => s.collection.id,
				);
				...
				pick.items = [
					...
					...Object.values(servers)
						.filter((s) => s.length)
						.flatMap(
							(servers): (ItemType | IQuickPickSeparator)[] => [
								{
									type: "separator",
...
export class McpServerOptionsCommand extends Action2 {
	...
	override async run(accessor: ServicesAccessor, id: string): Promise<void> {
		...
		const server = mcpService.servers
			.get()
		...
		switch (pick.action) {
			...
			case "config":
				editorService.openEditor({
					resource: URI.isUri(configTarget)
						? configTarget
						: configTarget!.uri,
					options: {
	...
	implements IWorkbenchContribution
{
	constructor(
		...
		const displayedState = derived((reader) => {
			...
			const unknownServerStates =
				mcpService.lazyCollectionState.read(reader);
					...
					return instaService.createInstance(
						class extends MenuEntryActionViewItem {
							override render(container: HTMLElement): void {
								...
								const action = h("button.chat-mcp-action", [
									h("span@icon"),
								]);
								...
								this._register(
									autorun((r) => {
										...
										if (state === DisplayedState.NewTools) {
											...
											state === DisplayedState.Refreshing
										) {
											root.classList.add(
							...
							private getLabelForState(
								{ state, servers } = displayedState.get(),
							) {
								if (state === DisplayedState.NewTools) {
									return localize(
										"mcp.newTools",
...
export class EditStoredInput extends Action2 {
	...
	run(
		...
		const workspaceFolder =
			uri &&
			accessor.get(IWorkspaceContextService).getWorkspaceFolder(uri);
		accessor
			.get(IMcpRegistry)
			.editSavedInput(
				inputId,
...
export class McpConfigureSamplingModels extends Action2 {
	...
	async run(accessor: ServicesAccessor, server: IMcpServer): Promise<number> {
		...
		const existingIds = new Set(
			mcpSampling.getConfig(server).allowedModels,
		);

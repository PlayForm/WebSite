...
export class McpAddConfigurationCommand {
	constructor(
		...
		@IMcpGalleryService
		private readonly _mcpGalleryService: IMcpGalleryService,
	...
	private async getServerType(): Promise<AddConfigurationType | undefined> {
		const items: QuickPickInput<
			{ kind: AddConfigurationType | "browse" } & IQuickPickItem
		>[] = [
			{
	...
	private async getStdioConfig(): Promise<
		IMcpConfigurationStdio | undefined
	> {
	...
	> {
		...
		const loadingAction = await new Promise<LoadAction | undefined>(
			(resolve) => {
	...
	private showOnceDiscovered(name: string) {
		...
		store.add(
			autorun((reader) => {
				...
				const server =
					match &&
					servers.find((s) => s.definition.id === match.server.id);
				...
				if (match && server) {
					if (match.collection.presentation?.origin) {
						this._openerService.openEditor({
							...
							options: {
								selection:
									match.server.presentation?.origin?.range,
								preserveFocus: true,
							},
						});
					} else {
	...
	private writeToUserSetting(
		name: string,
		config: McpConfigurationServer,
		target: ConfigurationTarget,
		inputs?: ConfiguredInput[],
	) {
		const settings: IMcpConfiguration = {
			...getConfigValueInTarget(
	...
	public async pickForUrlHandler(
		...
		const getEditors = () =>
			this._editorService
				...
				.filter(
					(e) =>
						e.editor.resource?.toString() === resource.toString(),
				);
			...
			case "install":
				...
				try {
					...
					const {
						inputs,
						...config
					}: McpConfigurationServer & { inputs?: ConfiguredInput[] } =
	...
	private getPackageType(
		serverType: AddConfigurationType,
	): string | undefined {
		switch (serverType) {
			case AddConfigurationType.NpmPackage:
				return "npm";

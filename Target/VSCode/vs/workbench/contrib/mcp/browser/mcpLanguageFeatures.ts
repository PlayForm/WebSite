	...
	implements IWorkbenchContribution
{
				...
				if (
					...
					diagnostics.push({
						...
						message: localize(
							...
							name,
							getClosestMatchingVariable(name) +
								(arg ? `:${arg}` : ""),
						),
	...
	private _provideCodeLenses(
		...
		for (const node of serversNode.children || []) {
			...
			const canDebug = !!server.readDefinitions().get().server?.devMode
				?.debug;
					...
					lenses.lenses.push(
						{
							...
							command: {
								...
								title:
									"$(loading~spin) " +
									localize("server.starting", "Starting"),
								arguments: [server.definition.id],
							},
						},
						{
					...
					lenses.lenses.push(
						{
							...
							command: {
								...
								title:
									"$(check) " +
									localize("server.running", "Running"),
								arguments: [server.definition.id],
							},
						},
						{
	...
	private async _provideInlayHints(
		...
		function annotateInputs(node: Node) {
			...
			for (const input of node.children) {
				...
				const idProp = input.children.find(
					(c) =>
						c.type === "property" && c.children?.[0].value === "id",
				);
				if (!idProp) {
					continue;
				}
		...
		function pushAnnotation(
			...
			const hint: InlayHint = {
				label:
					"= " +
					(saved.input?.type === "promptString" &&
					saved.input.password
						? "*".repeat(10)
						: saved.value || ""),
...
function forEachPropertyWithReplacement(
	...
	if (
		...
		node.children?.forEach((n) =>
			forEachPropertyWithReplacement(n, callback),
		);
	}
}

...
export class McpResourcePickHelper {
	...
	private async _resourceToAttachment(resource: {
		...
		const asImage = await resolveImageEditorAttachContext(
			this._fileService,
			this._dialogService,
			resource.uri,
			undefined,
	...
	private async _resourceTemplateToURI(rt: IMcpResourceTemplate) {
		...
		const initialCompletions = todo.map((variable) =>
			rt.complete(variable.name, "", {}, cts.token),
		);
	...
	private _promptForTemplateValue(
		...
		initialCompletions: Promise<string[]>,
		...
		let placeholder = localize(
			...
			variable.name.toUpperCase(),
			rt.template
				.resolve(variablesWithPlaceholders)
				.replaceAll("%24", "$"),
		);
	...
	public getPicks(
		...
		>();
		const addServerResources = async (
			server: IMcpServer,
			writeInto: (IMcpResourceTemplate | IMcpResource)[],
		) => {
			return Promise.all([
				(async () => {
		...
		store.add(
			picker.onDidTriggerItemButton((event) => {
				if (event.button.tooltip === attachButton && chatWidget) {
					...
					helper
						.toAttachment(
							(event.item as ResourceQuickPickItem).resource,
						)
						.then((a) => {
							if (a) {

...
export class ExtensionMcpDiscovery extends Disposable implements IMcpDiscovery {
	...
	private readonly cachedServers: {
		[collcetionId: string]: IServerCacheEntry;
	};
	...
	constructor(
		...
		this._register(
			storageService.onWillSaveState(() => {
				let updated = false;
				for (const collectionId of this
					._extensionCollectionIdsToPersist) {
					...
					for (const coll of collections.value) {
						...
						const dispo = this._mcpRegistry.registerCollection({
							...
							lazy: {
								isCached: !!serverDefs,
								load: () =>
									this._activateExtensionServers(coll.id),
								removed: () =>
	...
	private async _activateExtensionServers(
		collectionId: string,
	): Promise<void> {
	...
	private static _validate(
		user: extensionsRegistry.IExtensionPointUser<

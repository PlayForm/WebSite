	...
	implements IMcpDiscovery
{
	...
	protected setDetails(detailsDto: Dto<INativeMcpDiscoveryData> | undefined) {
		...
		const details: INativeMcpDiscoveryData = {
			...
			xdgHome: detailsDto.xdgHome
				? URI.revive(detailsDto.xdgHome)
				: undefined,
		...
		for (const adapter of this.adapters) {
			...
			const collection: WritableMcpCollectionDefinition = {
				id: adapter.id,
				label:
					discoverySourceLabel[adapter.discoverySource] + this.suffix,
				...
				presentation: {
					...
					order:
						adapter.order +
						(adapter.remoteAuthority
							? McpCollectionSortOrder.RemoteBoost
							: 0),
				},
			};

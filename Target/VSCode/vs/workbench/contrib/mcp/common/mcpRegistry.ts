...
export class McpRegistry extends Disposable implements IMcpRegistry {
	...
	public readonly collections: IObservable<
		readonly McpCollectionDefinition[]
	> = derived((reader) => {
		...
		if (toReplace) {
			...
			this._collections.set(
				[...currentCollections, collection].sort(
					(a, b) =>
						(a.presentation?.order || 0) -
						(b.presentation?.order || 0),
				),
				undefined,
			);
		}
	...
	private async _updateStorageWithExpressionInputs(
		...
		for (const [replacement, resolved] of expr.resolved()) {
			if (
				resolved.input?.type === "promptString" &&
				resolved.input.password
			) {
	...
	public async resolveConnection({
		...
		logger,
		debug,
	}: IMcpResolveConnectionOptions): Promise<
		IMcpServerConnection | undefined
	> {
		...
		try {
			...
			if (definition.devMode && debug) {
				launch = await this._instantiationService.invokeFunction(
					(accessor) =>
						accessor
							.get(IMcpDevModeDebugging)
							.transform(definition, launch!),
				);
			}
		} catch (e) {

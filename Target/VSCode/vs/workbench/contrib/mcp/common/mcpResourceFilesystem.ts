		...
		IFileSystemProviderWithFileReadStreamCapability
{
	...
	public watch(uri: URI, _opts: IWatchOptions): IDisposable {
		...
		store.add(
			autorun((reader) => {
				...
				handler
					.subscribe({ uri: resourceURI.toString(true) }, token)
					.then(
						() => {
							if (!token.isCancellationRequested) {
								watchListener.value =
									handler.onDidUpdateResource((e) => {
										if (
											equalsUriPath(
												e.params.uri,
												resourceURI,
	...
	public async readdir(resource: URI): Promise<[string, FileType][]> {
		const { forSameURI, contents, resourceURI } =
			await this._readURI(resource);
	...
	private _decodeURI(uri: URI) {
		...
		if (resourceURI.path.endsWith("/")) {
			resourceURI = resourceURI.with({
				path: resourceURI.path.slice(0, -1),
			});
		}
	...
	private async _readURI(uri: URI, token?: CancellationToken) {
		...
		const res = await McpServer.callOn(
			server,
			(r) => r.readResource({ uri: resourceURI.toString(true) }, token),
			token,
		);
		...
		return {
			...
			resourceURI,
			forSameURI: res.contents.filter((c) =>
				equalsUriPath(c.uri, resourceURI),
			),
		};
	}
}

	...
	implements IMcpSamplingService
{
	...
	constructor(
		...
		super();
		this._logs = this._register(
			instaService.createInstance(McpSamplingLog),
		);
	}
	...
	async sample(
		...
		const messages = opts.params.messages
			.map((message): IChatMessage | undefined => {
				const content: IChatMessagePart | undefined =
	...
	private async _getMatchingModelInner(
		...
		const foundModelIdsDeep =
			...
			this._languageModelsService
				...
				.filter(
					(m) =>
						this._languageModelsService.lookupLanguageModel(m)
							?.isDefault,
				);

		const foundModelIds = foundModelIdsDeep
			.flat()
			.sort((a, b) => b.length - a.length); // Sort by length to prefer most specific
	...
	public async updateConfig(
		...
		const { value, mapping, key, target, resource } =
			this._getConfig(server);

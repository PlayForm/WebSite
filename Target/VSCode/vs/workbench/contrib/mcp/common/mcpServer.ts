...
class CachedPrimitive<T, C> {
	...
	private readonly fromServer = derived(
		(reader) =>
			this.fromServerPromise.read(reader)?.promiseResult.read(reader)
				?.data,
	);
...
export class McpServer extends Disposable implements IMcpServer {
	...
	public readonly cacheState = derived((reader) => {
		...
		const isIdle =
			McpConnectionState.canBeStarted(connectionState.state) ||
			!fromServer;
		if (isIdle) {
	...
	constructor(
		...
		@IMcpSamplingService
		private readonly _samplingService: IMcpSamplingService,
	) {
		super();
			...
			async (emitter) => {
				await McpServer.callOn(this, async (handler) => {
					for await (const resource of handler.listResourcesIterable(
						...
						emitter.emitOne(
							resource.map((r) => new McpResource(this, r)),
						);
	...
	private populateLiveData(
		...
		store.add(
			handler.onDidChangePromptList(() => {
				handler.logger.info(
					"Prompts list changed, refreshing prompts...",
				);
...
class McpPrompt implements IMcpPrompt {
	...
	async resolve(
		...
		const result = await McpServer.callOn(
			...
			(h) =>
				h.getPrompt(
					{ name: this._definition.name, arguments: args },
					token,
				),
			token,
		);
				...
				h.complete(
					{
						...
						resolved: mapValues(alreadyResolved, (v) =>

...
export class McpServerRequestHandler extends Disposable {
	...
	private readonly _onDidChangePromptList = this._register(
		new Emitter<void>(),
	);
	...
	>(
		...
		do {
			...
			const result: R = await this.sendRequest<T, R>(
				{ method, params },
				token,
			);
		...
		if ("method" in message) {
			if ("id" in message) {
				this.handleServerRequest(
					message as MCP.JSONRPCRequest & MCP.ServerRequest,
				);
			} else {
				this.handleServerNotification(
					message as MCP.JSONRPCNotification & MCP.ServerNotification,
				);
			}
		}
	}
	...
	private async handleServerRequest(
		...
		try {
			...
			if (request.method === "ping") {
				...
				response = await this._createMessageRequestHandler(
					request.params as MCP.CreateMessageRequest["params"],
				);
			} else {
			...
			if (!(e instanceof McpError)) {
				this.logger.error(
					`Error handling request ${request.method}:`,
					e,
				);
	...
	listResources(
		...
		return Iterable.asyncToArrayFlat(
			this.listResourcesIterable(params, token),
		);
	}
	...
	readResource(
		...
		>({ method: "resources/read", params }, token);
	}
	 ...
	 */
	complete(
		params: MCP.CompleteRequest2["params"],
		token?: CancellationToken,
	): Promise<MCP.CompleteResult> {
		return this.sendRequest<MCP.CompleteRequest2, MCP.CompleteResult>(

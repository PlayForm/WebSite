	...
	public updateCollectedServers() {
			...
			.flatMap((collectionDefinition) =>
				collectionDefinition.serverDefinitions
					...
					.map((serverDefinition) => {
						const toolPrefix = prefixGenerator.generate(
		...
		for (const server of currentServers) {
			const match = definitions.find(
				(d) =>
					defsEqual(server.object, d) &&
					server.toolPrefix === d.toolPrefix,
			);
			if (match) {

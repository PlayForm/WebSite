...
suite("Workbench - MCP - ResourceFilesystem", () => {
	...
	setup(() => {
		...
		const parentInsta2 = ds.add(
			parentInsta1.createChild(
				new ServiceCollection([IMcpRegistry, registry]),
			),
		);
	...
	test("readdir returns directory contents", async () => {
			...
			return {
				...
				result: {
					contents: [
						...
						{
							uri: "custom://hello/dir/subdir/file3.txt",
							text: "File 3",
						},
					],
				} satisfies MCP.ReadResourceResult,
		...
		const dirEntries = await fs.readdir(
			URI.parse(
				"mcp-resource://746573742D736572766572/custom/hello/dir/",
			),
		);
	...
	test("readdir throws when reading a file as directory", async () => {
		...
		await assert.rejects(
			...
			(err: any) =>
				err.code === FileSystemProviderErrorCode.FileNotADirectory,
		);
	});
	...
	test("watch file emits change events", async () => {
		...
		const watchDisposable = fs.watch(uri, {
			excludes: [],
			recursive: false,
		});
	...
	test("read blob resource", async () => {
		...
		const response = await fs.readFile(
			URI.parse(
				"mcp-resource://746573742D736572766572/custom/hello/blob.bin",
			),
		);
		assert.strictEqual(
	...
	test("throws error for write operations", async () => {
		...
		await assert.rejects(
			...
			(err: any) =>
				err.code === FileSystemProviderErrorCode.NoPermissions,
		);

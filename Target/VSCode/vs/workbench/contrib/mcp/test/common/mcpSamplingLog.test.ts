...
suite("MCP - Sampling Log", () => {
	...
	test("shifts bins when adding requests on different days", async () => {
		...
		log.add(
			...
			[
				{
					role: "user",
					content: { type: "text", text: "day 1 request" },
				},
			],
			"day 1 response",
			"foobar9000",
		...
		log.add(
			...
			[
				{
					role: "user",
					content: { type: "text", text: "day 2 request" },
				},
			],
			"day 2 response",
			"foobar9000",
		...
		log.add(
			...
			[
				{
					role: "user",
					content: { type: "text", text: "day 7 request" },
				},
			],
			"day 7 response",
			"foobar9000",
	...
	test("limits the number of stored requests", async () => {
		...
		assert.strictEqual(
			data.lastReqs[29].request[0].content.text,
			"request 5",
		);
	});
	...
	test("handles multiple servers", async () => {
		const fakeServer2: IMcpServer = {
			...
			readDefinitions: () => ({
				get: () => ({
					collection: { scope: StorageScope.APPLICATION },
				}),
			}),
		} as any;
		...
		log.add(
			...
			[
				{
					role: "user",
					content: { type: "text", text: "server1 request" },
				},
			],
			"server1 response",
			"foobar9000",
		...
		log.add(
			...
			[
				{
					role: "user",
					content: { type: "text", text: "server2 request" },
				},
			],
			"server2 response",
			"foobar9000",

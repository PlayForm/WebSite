...
export class MergeEditorModel extends EditorModel {
	...
	constructor(
		...
		this.modifiedBaseRanges = derived<ModifiedBaseRange[]>(
			this,
			(reader) => {
				const input1Diffs =
					this.input1TextModelDiffs.diffs.read(reader);
				const input2Diffs =
					this.input2TextModelDiffs.diffs.read(reader);
...
class ModifiedBaseRangeData {
	constructor(private readonly baseRange: ModifiedBaseRange) {
		...
		this.handled = derived(
			this,
			(reader) =>
				this.handledInput1.read(reader) &&
				this.handledInput2.read(reader),
		);
	}

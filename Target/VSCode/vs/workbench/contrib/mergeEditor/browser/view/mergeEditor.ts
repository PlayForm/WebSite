...
export class MergeEditor extends AbstractTextEditor<IMergeEditorViewState> {
	...
	constructor(
		...
		this._ctxIsMergeEditor = ctxIsMergeEditor.bindTo(
			this.contextKeyService,
		);
		this._ctxUsesColumnLayout = ctxMergeEditorLayout.bindTo(
			this.contextKeyService,
		);
		this._ctxShowBase = ctxMergeEditorShowBase.bindTo(
			this.contextKeyService,
		);
		...
		this._ctxShowNonConflictingChanges =
			ctxMergeEditorShowNonConflictingChanges.bindTo(
				this.contextKeyService,
			);
		...
		this.onDidChangeSizeConstraints =
		...
		this._sessionDisposables.add(
			autorunWithStore((reader) => {
				...
				const resultScrollTop =
					this.inputResultView.editor.getScrollTop();

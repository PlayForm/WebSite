...
export class MergeEditorViewModel extends Disposable {
	...
	constructor(
		...
		this.baseShowDiffAgainst = derived<1 | 2 | undefined>(
			this,
			(reader) => {
				...
				if (lastFocusedEditor.view === this.inputCodeEditorView1) {
					...
					lastFocusedEditor.view === this.inputCodeEditorView2
				) {
					return 2;
				}
				return undefined;
		...
		this.focusedEditorType = derived<MergeEditorType | undefined>(
			...
			(reader) => {
				...
				if (lastFocusedEditor.view === this.inputCodeEditorView1) {
					...
					lastFocusedEditor.view === this.resultCodeEditorView
				) {
					return "result";
				} else if (
		...
		this.selectionInBase = derived(this, (reader) => {
			...
			const rangesInBase = selections.map((selection) => {
				if (sourceEditor === this.inputCodeEditorView1) {
					...
					sourceEditor === this.baseCodeEditorView.read(reader)
				) {
					return selection;
				} else {
					return selection;
		...
		this.activeModifiedBaseRange = derived(this, (reader) => {
			...
			const modifiedBaseRanges =
				this.model.modifiedBaseRanges.read(reader);

	...
	implements INotebookEditorContribution
{
				...
				(e) => {
					if (
						...
						if (
							this.configurationService.getValue<
								"on" | "auto" | "off"
							>(NotebookSetting.notebookInlineValues) === "off"
						) {
			...
			const promises = providers.flatMap((provider) =>
				Promise.resolve(
					...
					async (result) => {
						...
						const kernel =
							this.notebookKernelService.getMatchingKernel(
								notebook,
							);
						...
						for (const iv of result) {
							...
							switch (iv.type) {
								...
								case "variable": {
									const name = (
										iv as InlineValueVariableLookup
									).variableName;
									if (!name) {
							...
							if (text) {
								...
								if (
									!lineSegments.some((iv) => iv.text === text)
								) {
									// de-dupe
									lineSegments.push(
										new InlineSegment(
			...
			for (const varName of varNames) {
				...
				if (lastMatchOutsideIgnored) {
					const inlineVal =
						varName +
						" = " +
						vars.find((v) => v.name === varName)?.value;
		...
		for (let lineNumber = 1; lineNumber <= lineCount; lineNumber++) {
			...
			for (
				...
				if (
					...
					const isLastToken =
						tokenIndex === lineTokens.getCount() - 1;
					const nextTokenDifferent =
						!isLastToken &&
						lineTokens.getStandardTokenType(tokenIndex + 1) !==
							tokenType;
	...
	override dispose(): void {
		super.dispose();
		this._clearNotebookInlineDecorations();
		this.currentCancellationTokenSources.forEach((source) =>
			source.cancel(),
		);

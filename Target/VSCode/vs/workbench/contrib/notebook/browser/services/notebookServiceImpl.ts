		...
		for (const selector of notebookProviderInfo.selectors) {
			...
			const notebookEditorInputFactory: EditorInputFactoryFunction =
				async ({ resource, options }) => {
					...
					if (resource.scheme === Schemas.vscodeNotebookCellOutput) {
						const outputUriData =
							CellUri.parseCellOutputUri(resource);
						if (
							!outputUriData ||
					...
					if (data) {
						...
						notebookUri =
							this.uriIdentService.asCanonicalUri(resource);
					}
					...
					if (resource.scheme === Schemas.vscodeNotebookCellOutput) {
						...
						const cellIndex =
							await this._notebookEditorModelResolverService
								.resolve(notebookUri)
								.then((model) =>
...
export class NotebookService extends Disposable implements INotebookService {
	...
	constructor(
		...
		this.onDidChangeOutputRenderers =
			this._onDidChangeOutputRenderers.event;
		this._notebookStaticPreloadInfoStore =
		...
		this.onDidRemoveNotebookDocument =

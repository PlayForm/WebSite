...
export class NotebookOverviewRuler extends Themable {
	...
	private _render(
		...
		if (viewModel) {
			...
			const overviewRulerDecorations =
			...
			for (let i = 0; i < overviewRulerDecorations.length; i++) {
				...
				const fillStyle =
					this.getColor(decoration.options.overviewRuler.color) ??
					"#000000";
				let x = 0;
				...
				const viewZoneTop =
					(viewZoneInfo.top / scrollHeight) * ratio * height;

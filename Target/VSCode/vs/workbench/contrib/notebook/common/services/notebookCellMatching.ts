...
export function matchCellBasedOnSimilarties(
	...
	results.forEach((result, i) => {
		...
		const previousMatchedCell =
			i > 0
				? results
						...
						.find((r) => r.original >= 0)
				: undefined;
		const previousMatchedOriginalIndex =
			previousMatchedCell?.original ?? -1;
		const previousMatchedModifiedIndex =
			previousMatchedCell?.modified ?? -1;
		...
		const { index, percentage } = computeClosestCell(
			...
			(
				originalIndex: number,
				originalValue: { editCount: EditCount },
			) => {
		...
		const nextOriginalCellValue =
			i > 0 &&
			nextOriginalCell >= 0 &&
			nextOriginalCell < originalCells.length

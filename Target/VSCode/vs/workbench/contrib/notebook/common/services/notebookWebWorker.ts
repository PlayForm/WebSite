...
class CellSequence implements ISequence {
	...
	static createWithCellId(
		cells: MirrorCell[],
		includeCellContents?: boolean,
	) {
			...
			if (!change.originalLength && change.modifiedLength) {
				...
				cellDiffInfo.slice(processedIndex, changeIndex).forEach((c) => {
					if (c.type === "unchanged" || c.type === "modified") {
						const originalCell =
							original.cells[c.originalCellIndex];
						const modifiedCell =
							modified.cells[c.modifiedCellIndex];
						const changed =
							c.type === "modified" ||
	...
	updateCellIdsBasedOnMappings(
		...
		originalCells.map((cell, index) => {
			...
			if (found) {
				...
				uuids.set(
					found.modified,
					cell.internalMetadata.internalId as string,
				);
			}
		});

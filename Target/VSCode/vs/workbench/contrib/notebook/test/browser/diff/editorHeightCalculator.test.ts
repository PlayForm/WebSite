...
suite("NotebookDiff EditorHeightCalculator", () => {
	["Hide Unchanged Regions", "Show Unchanged Regions"].forEach(
		(suiteTitle) => {
			suite(suiteTitle, () => {
					...
					editorWorkerService =
						new (class extends mock<IEditorWorkerService>() {
							override async computeDiff(
								...
								const originalLines = new Array(
									originalModel.getLineCount(),
								)
									.fill(0)
									.map((_, i) =>
										originalModel.getLineContent(i + 1),
									);
								const modifiedLines = new Array(
									modifiedModel.getLineCount(),
								)
									.fill(0)
									.map((_, i) =>
										modifiedModel.getLineContent(i + 1),
									);
				...
				test("1 original line with change in same line", async () => {
					originalModel = disposables.add(
						createTextModel(["Hello World"]),
					);
					modifiedModel = disposables.add(
						createTextModel(["Foo Bar"]),
					);
				...
				test("10 line with update to a line and insert of a new line", async () => {
					originalModel = disposables.add(
						createTextModel(createLines(10)),
					);
					modifiedModel = disposables.add(
				...
				test("50 lines with updates, deletions and inserts", async () => {
					originalModel = disposables.add(
						createTextModel(createLines(60)),
					);

...
class OutputContribution extends Disposable implements IWorkbenchContribution {
			...
			registerAction2(
				class extends Action2 {
					...
					async run(accessor: ServicesAccessor): Promise<void> {
						...
						const entries: Array<IOutputChannelDescriptor> =
							outputService
	...
	private async openActiveOutput(
						...
						async run(accessor: ServicesAccessor): Promise<void> {
							...
							if (channel) {
								const channelDescriptor =
									outputService.getChannelDescriptor(
										channel.id,
									);
			...
			registerAction2(
				class extends Action2 {
					...
					async run(accessor: ServicesAccessor): Promise<void> {
						...
						if (channel) {
							const channelDescriptor =
								outputService.getChannelDescriptor(channel.id);
							if (
								channelDescriptor &&
								isSingleSourceOutputChannelDescriptor(
									channelDescriptor,
				...
				registerAction2(
					class extends ViewAction<OutputViewPane> {
						...
						private toggleLogLevelFilter(
							...
							switch (logLevel) {
								case LogLevel.Trace:
									outputService.filters.trace =
										!outputService.filters.trace;
									break;
								case LogLevel.Debug:
									outputService.filters.debug =
										!outputService.filters.debug;
									break;
								case LogLevel.Info:
									outputService.filters.info =
										!outputService.filters.info;
									break;
			...
			registerAction2(
				class extends Action2 {
					...
					async run(accessor: ServicesAccessor): Promise<void> {
						...
						if (
							userLogs.length &&
							(extensionLogs.length || logs.length)
						) {
							entries.push({
								type: "separator",

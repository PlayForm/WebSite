	...
	private onDidChangeModel(): void {
		...
		const computeEndLineNumber = () => {
			...
			return endLineNumber > 1 &&
				model.getLineMaxColumn(endLineNumber) === 1
				? endLineNumber - 1
				: endLineNumber;
		};
		...
		this.modelDisposables.add(
			model.onDidChangeContent((e) => {
				if (
					e.changes.every(
						(e) => e.range.startLineNumber > endLineNumber,
					)
				) {
	...
	private compute(
		...
		if (activeChannel && logEntries?.length) {
			...
			for (let i = fromLogLevelEntryIndex; i < logEntries.length; i++) {
				...
				if (
					hasLogLevelFilter &&
					!this.shouldShowLogLevel(entry, filters)
				) {
					hiddenAreas.push(entry.range);
					continue;
				}
				if (
					!this.shouldShowCategory(activeChannel.id, entry, filters)
				) {
					hiddenAreas.push(entry.range);
					continue;
				}

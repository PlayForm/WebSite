...
export function parseLogEntryAt(
	...
	if (match) {
		...
		while (endLine < lineCount) {
			...
			const isLastLine =
				endLine + 1 === lineCount && nextLineContent === ""; // Last line will be always empty
...
class MultiFileContentProvider extends Disposable implements IContentProvider {
	...
	async getContent(): Promise<{
		...
		const outputs = await Promise.all(
			this.fileContentProviderItems.map(([output]) =>
				output.getContent(true),
			),
		);
				...
				while (!next.done) {
					...
					if (
						...
						for (
							next = iterator.next();
							!next.done &&
							next.value[0].timestamp <=
								logEntries[insertionIndex].timestamp;
							next = iterator.next()
						) {
							logEntriesToAdd.push(next.value[0]);
							contentsToAdd.push(next.value[1]);

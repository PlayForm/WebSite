...
class EmbeddingsSearchProvider implements IRemoteSearchProvider {
	...
	async searchModel(
		...
		this._aiSettingsSearchService.startSearch(
			this._filter,
			this._excludeSelectionStep,
			token,
		);
	...
	private async getEmbeddingsItems(
		...
		const settings =
			await this._aiSettingsSearchService.getEmbeddingsResults(
				this._filter,
				token,
			);
		if (!settings) {
		...
		const providerName = this._excludeSelectionStep
			? "embeddingsOnly"
			: "embeddingsFull";
...
class RemoteSearchProvider implements IRemoteSearchProvider {
	...
	async searchModel(
		...
		if (!this.aiSettingsSearchService.isEnabled()) {
			return this._tfIdfSearchProvider.searchModel(
				preferencesModel,
				token,
			);
		}

		let results = await this._embeddingsSearchProvider.searchModel(
			preferencesModel,
			token,
		);
		...
		if (!token.isCancellationRequested) {
			results = await this._tfIdfSearchProvider.searchModel(
				preferencesModel,
				token,
			);
			if (results?.filterMatches.length) {

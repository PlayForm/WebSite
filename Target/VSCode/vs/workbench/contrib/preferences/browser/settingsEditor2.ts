...
export class SettingsEditor2 extends EditorPane {
	...
	toggleAiSearch(): void {
		if (this.showAiResultsAction && this.showAiResultsAction.enabled) {
			this.showAiResultsAction.checked =
				!this.showAiResultsAction.checked;
		}
	}
		...
		return this.configurationService
			...
			.then(() => {
				...
				const reportModifiedProps = {
					...
					query,
					searchResults:
						this.searchResultModel?.getUniqueSearchResults() ??
						null,
	...
	private async triggerSearch(
		...
		if (query && query !== "@") {
			...
			await this.triggerFilterPreferences(
				query,
				expandResults,
				progressRunner,
			);
			...
			return this.searchDelayer.trigger(async () => {
				...
				if (embeddingsResults?.filterMatches.length) {
					this.aiSearchDelayer.trigger(async () => {
						await this.getLLMRankedResults(
							query,
							searchInProgress.token,
						);
						if (searchInProgress.token.isCancellationRequested) {

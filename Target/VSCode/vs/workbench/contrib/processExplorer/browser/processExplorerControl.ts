...
class ProcessHeaderTreeRenderer
	implements
		ITreeRenderer<IProcessInformation, void, IProcessItemTemplateData>
{
	...
	renderElement(
		...
		templateData.memory.textContent = localize(
			"processMemory",
			"Memory (MB)",
		);
	}
	...
	implements ITreeRenderer<ProcessItem, void, IProcessItemTemplateData>
{
	...
	renderElement(
		...
		templateData.memory.textContent = (element.mem / ByteSize.MB).toFixed(
			0,
		);
	...
	private createProcessTree(container: HTMLElement): void {
		...
		this._register(
			this.tree.onContextMenu((e) =>
				this.onTreeContextMenu(container, e),
			),
		);
	...
	private onTreeContextMenu(
			...
			toAction({
				...
				run: () => {
					...
					const rows = selectionPids
						?.map((e) =>
							getDocument(container).getElementById(`pid-${e}`),
						)
						.filter((e) => !!e);
					if (rows) {
						const text = rows
		...
		actions.push(
			toAction({
				...
				run: () => {
					const processList =
						getDocument(container).getElementById(
							"process-explorer",
						);
					if (processList) {

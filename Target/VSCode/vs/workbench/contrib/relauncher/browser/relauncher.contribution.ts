	...
	implements IWorkbenchContribution
{
	...
	private update(askToRelaunch: boolean): void {
		...
		if (isNative) {
			...
			processChanged(
				!isMacintosh &&
					this.menuStyle.handleChange(config.window?.menuStyle),
			);
			...
			processChanged(
				!isMacintosh &&
					this.controlsStyle.handleChange(
		...
		processChanged(
			this.showSuggestions.handleChange(
				config.workbench?.settings?.showSuggestions,
			),
		);

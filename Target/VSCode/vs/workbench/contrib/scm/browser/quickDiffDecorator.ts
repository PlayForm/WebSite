...
class QuickDiffDecorator extends Disposable {
	...
	constructor(
		...
		this.modifiedSecondaryPatternOptions =
			QuickDiffDecorator.createDecoration(
		...
		for (const change of this.quickDiffModelRef.object.changes) {
			...
			if (
				...
				primaryQuickDiffChanges.some((c) =>
					c.change2.modified.intersectsOrTouches(
						change.change2.modified,
					),
				)
			) {
				...
				const visibleDecorationCount = observableFromEvent(
					...
					() => {
						const visibleQuickDiffs =
							quickDiffModelRef.object.quickDiffs.filter(
								(quickDiff) =>
						...
						return quickDiffModelRef.object.changes.filter(
							(change) =>
								visibleQuickDiffs.some(
									(quickDiff) =>
										quickDiff.id === change.providerId,
								),
						).length;
					},
				);
						...
						registerAction2(
							class extends Action2 {
								...
								override run(accessor: ServicesAccessor): void {
									const quickDiffService =
										accessor.get(IQuickDiffService);

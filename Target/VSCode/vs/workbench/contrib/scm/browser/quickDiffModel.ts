...
export class QuickDiffModel extends Disposable {
	...
	private diff(): Promise<{
		...
		return this.progressService.withProgress(
			...
			async () => {
						...
						for (
							...
							if (
								...
								const primaryQuickDiffChange = diffs.find(
									(d) =>
										d.change2.modified.equals(
											change2.modified,
										) &&
										d.change2.original.length ===
											change2.original.length,
								);

								if (primaryQuickDiffChange) {
									// Check whether the original content matches
									const primaryModel =
										this._originalEditorModels.get(
											quickDiffPrimary.originalResource,
										)?.textEditorModel;
									const primaryContent =
										primaryModel?.getValueInRange(
									...
									const secondaryContent =
										secondaryModel?.getValueInRange(
											change2.toRangeMapping()
												.originalRange,
										);
				...
				const allDiffsSorted = [...diffs, ...secondaryDiffs].sort(
	...
	findNextClosestChange(
		...
		const visibleQuickDiffIds = this.quickDiffs
			.filter(
				(quickDiff) =>
					(!providerId || quickDiff.id === providerId) &&
					this.quickDiffService.isQuickDiffProviderVisible(
						quickDiff.id,
					),
			)
		...
		if (!inclusive) {
			// Next visible change
			let nextChangeIndex = this.changes.findIndex(
				(change) =>
					visibleQuickDiffIds.includes(change.providerId) &&
			...
			nextChangeIndex = this.changes.findIndex((change) =>
				visibleQuickDiffIds.includes(change.providerId),
			);
		...
		const primaryQuickDiffId = this.quickDiffs.find(
			(quickDiff) => quickDiff.kind === "primary",
		)?.id;

		const primaryInclusiveChangeIndex = this.changes.findIndex(
			(change) =>
				change.providerId === primaryQuickDiffId &&
				change.change.modifiedStartLineNumber <= lineNumber &&
	...
	findPreviousClosestChange(
		...
		for (let i = this.changes.length - 1; i >= 0; i--) {
			...
			const quickDiff = this.quickDiffs.find(

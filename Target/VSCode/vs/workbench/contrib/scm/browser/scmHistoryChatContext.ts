			...
			picks: picksWithPromiseFn(
				(query: string, token: CancellationToken) => {
					...
					const activeRepository =
						this._scmViewService.activeRepository.get();
					const historyProvider =
						activeRepository?.provider.historyProvider.get();
							...
							.then((historyItems) => {
								...
								return historyItems.map((historyItem) => {
									...
									return {
										...
										detail: details.join(
											`$(${Codicon.circleSmallFilled.id})`,
										),
										asAttachment: () =>
											SCMHistoryItemContext.asAttachment(
												activeRepository.provider,
												historyItem,
											),
...
class SCMHistoryItemContextContentProvider

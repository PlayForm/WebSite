		...
		>
{
	...
	private _renderGraphPlaceholder(
		...
		const graphPlaceholderSvgWidth =
			SWIMLANE_WIDTH * (graphColumns.length + 1);
		const marginLeft =
...
class SCMHistoryTreeIdentityProvider implements IIdentityProvider<TreeElement> {
	getId(element: TreeElement): string {
		if (isSCMRepository(element)) {
			...
			const historyItem =
				element.context.historyItemViewModel.historyItem;
					...
					...historyItemChanges.map(
						(change) =>
							({
								...
								historyItemViewModel:
									inputOrElement.historyItemViewModel,
...
export class SCMHistoryViewPane extends ViewPane {
			...
			async (visible) => {
				...
				this._visibilityDisposables.add(
					autorun((reader) => {
						...
						const viewMode =
							this._treeViewModel.viewMode.read(reader);
		...
		if (!e.element) {
			...
			const historyItemParentDisplayId =
				historyItemParentId && historyItem.displayId
					? historyItemParentId.substring(
							0,
							historyItem.displayId.length,
						)
					: historyItemParentId;

			if (
				historyItemChange.originalUri &&
				historyItemChange.modifiedUri
			) {
				// Diff Editor
	...
	private _onContextMenu(e: ITreeContextMenuEvent<TreeElement | null>): void {
		...
		if (
			...
			for (const ref of element.historyItemViewModel.historyItem
				.references) {

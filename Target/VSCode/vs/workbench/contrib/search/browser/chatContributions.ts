...
{
	static readonly ID =
		"workbench.contributions.searchChatContextContribution";

	constructor(
		@IInstantiationService instantiationService: IInstantiationService,
		@IChatContextPickService
		chatContextPickService: IChatContextPickService,
	) {
		super();
		this._store.add(
	...
	asPicker() {
		return {
			...
			picks: picksWithPromiseFn(
				(query: string, token: CancellationToken) => {
					this._provider ??=
	...
	asPicker() {
		return {
			...
			picks: picksWithPromiseFn(async (value, token) => {
				...
				(
					await getTopLevelFolders(workspaces, this._fileService)
				).forEach((uri) =>
					defaultItems.push(
						this._createPickItem(uri, FileKind.FOLDER),
					),
				);
				...
				await Promise.all(
					workspaces.map(async (workspace) => {
						...
						for (const folder of folders) {
							result.push(
								this._createPickItem(folder, FileKind.FOLDER),
							);
						}
						for (const file of files) {
							result.push(
								this._createPickItem(file, FileKind.FILE),
							);
						}
					}),
				);

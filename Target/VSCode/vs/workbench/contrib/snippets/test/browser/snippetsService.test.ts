...
suite("SnippetsService", function () {
	...
	test("snippet completions - with different prefixes", async function () {
...

		{
			await provider
				...
				.then((result) => {
					...
					assert.strictEqual(
						(result.suggestions[0].range as CompletionItemRanges)
							.insert.startColumn,
						1,
					);
					...
					assert.strictEqual(
						(result.suggestions[1].range as CompletionItemRanges)
							.insert.startColumn,
						1,
					);
				});
			...
			assert.strictEqual(
				(completions.items[0].completion.range as CompletionItemRanges)
					.insert.startColumn,
				1,
			);
			...
			assert.strictEqual(
				(completions.items[1].completion.range as CompletionItemRanges)
					.insert.startColumn,
				1,
			);
		}

		{
			await provider
				...
				.then((result) => {
					...
					assert.strictEqual(
						(first.range as CompletionItemRanges).insert
							.startColumn,
						5,
					);
					...
					assert.strictEqual(
						(second.range as CompletionItemRanges).insert
							.startColumn,
						1,
					);
				});
...

		{
			...
			assert.strictEqual(
				(completions.items[0].completion.range as any).insert
					.startColumn,
				1,
			);
			assert.deepStrictEqual(completions.items[1].completion.label, {
			...
			assert.strictEqual(
				(completions.items[1].completion.range as any).insert
					.startColumn,
				5,
			);
		}

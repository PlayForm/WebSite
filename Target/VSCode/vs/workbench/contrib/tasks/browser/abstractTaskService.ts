	...
	protected _showOutput(
		...
		if (
			...
			if (userRequested) {
				...
				const chatEnabled = this._chatService.isEnabled(
	...
	private _handleInstancePolicy(task: Task, policy?: InstancePolicy): void {
		...
		switch (policy) {
			case InstancePolicy.terminateNewest:
				this._restart(
					this._getTaskSystem().getLastInstance(task) ?? task,
				);
				break;
			case InstancePolicy.terminateOldest:
				this._restart(
					this._getTaskSystem().getFirstInstance(task) ?? task,
				);
				break;
			...
			default:
				this._showQuickPick(
					this._taskSystem!.getActiveTasks().filter(
						(t) => task._id === t._id,
					),
					nls.localize(

	...
	implements ITerminalContribution
{
	...
	private readonly _pwshAddon: MutableDisposable<PwshCompletionProviderAddon> =
		new MutableDisposable();
	private readonly _lspAddon: MutableDisposable<LspCompletionProviderAddon> =
		new MutableDisposable(); // TODO: Support multiple lspAddons in the future.
	...
	get lspAddon(): LspCompletionProviderAddon | undefined {
	...
	private async _loadPwshCompletionAddon(
	...
	private async _loadLspCompletionAddon(
		...
		const isWsl =
			isLinux && (!!env["WSL_DISTRO_NAME"] || !!env["WSL_INTEROP"]);
		...
		const provider = virtualProviders.find(
			(p) => p._debugDisplayName === PYLANCE_DEBUG_DISPLAY_NAME,
		);

		if (provider) {
			const lspCompletionProviderAddon = (this._lspAddon.value =
				this._instantiationService.createInstance(

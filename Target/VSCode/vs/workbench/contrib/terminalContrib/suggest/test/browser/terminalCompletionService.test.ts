...
suite("TerminalCompletionService", () => {
	...
	suite("~ -> $HOME", () => {
		...
		setup(() => {
			...
			childResources = [
				{
					resource: URI.parse("file:///home/vscode"),
					isDirectory: true,
				},
				{
					resource: URI.parse("file:///home/vscode/foo"),
					isDirectory: true,
				},
				{
	...
	suite("resolveResources edge cases and advanced scenarios", () => {
		...
		if (isWindows) {
			...
			test("./folder | Case sensitivity should resolve correctly on Mac/Unix", async () => {
				...
				childResources = [
					{
						resource: URI.parse("file:///test/FolderA/"),
						isDirectory: true,
					},
					{
						resource: URI.parse("file:///test/foldera/"),
		...
		test("| Empty input should resolve to current directory", async () => {
			...
			childResources = [
				...
				{
					resource: URI.parse("file:///test/folder2/"),
					isDirectory: true,
				},
			];
			const result = await terminalCompletionService.resolveResources(
	...
	suite("cdpath", () => {
		...
		setup(() => {
			...
			childResources = [
				...
				{
					resource: URI.parse("file:///cdpath_value/file1.txt"),
					isFile: true,
				},
			];
		...
		test("cd | should support pulling from multiple paths in $CDPATH", async () => {
			...
			childResources = [
				{
					resource: URI.parse(
						`${uriPathPrefix}cdpath1_value/folder1/`,
					),
					isDirectory: true,
				},
				{
					resource: URI.parse(
						`${uriPathPrefix}cdpath1_value/folder2/`,
					),
					isDirectory: true,
				},
				{
					resource: URI.parse(
						`${uriPathPrefix}cdpath1_value/file1.txt`,
					),
					isFile: true,
				},
				{

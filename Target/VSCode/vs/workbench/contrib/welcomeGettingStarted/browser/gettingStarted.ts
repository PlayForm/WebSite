...
export class GettingStartedPage extends EditorPane {
	...
	override async setInput(
		...
		this.editorInput.showTelemetryNotice =
			(options as GettingStartedEditorOptions)?.showTelemetryNotice ??
			true;
	...
	private async buildCategoriesSlide() {
		...
		if (
			...
			if (fistContentBehaviour === "openToFirstCategory") {
				const exp =
					this.gettingStartedExperimentService.getCurrentExperiment();
		...
		allSlides.forEach((slide, index) => {
			...
			if (slide.steps.length === 1) {
				...
				slide.steps.forEach((step, i) => {
					...
					this.buildMarkdownDescription(subStepDesc, [
						step.description[0],
					]);
				...
				const linkedText =
					lastStep.description.length > 1
						? lastStep.description[1]
						: undefined;
				if (linkedText) {

...
export class NullDefaultAccountService
	extends Disposable
	implements IDefaultAccountService
{
	declare _serviceBrand: undefined;
	...
	implements IWorkbenchContribution
{
			...
			this.authenticationService.onDidChangeSessions(async (e) => {
				...
				if (
					...
					e.event.removed?.some(
						(session) =>
							session.id === this.defaultAccount?.sessionId,
					)
				) {
	...
	private async getDefaultAccountFromAuthenticatedSessions(
		...
		const id =
			this.configurationService.getValue(enterpriseAuthProviderConfig) ===
		...
		const session = sessions.find((s) =>
			this.scopesMatch(s.scopes, scopes),
		);
	...
	private async getTokenEntitlements(
		...
		try {
			...
			const chatData =
				await asJson<ITokenEntitlementsResponse>(chatContext);
			if (chatData) {
				return {
					// Editor preview features are disabled if the flag is present and set to 0
					chat_preview_features_enabled:
						this.extractFromToken(
							chatData.token,
						...
						super({
							...
							menu: {
								...
								when: ContextKeyExpr.and(
									CONTEXT_DEFAULT_ACCOUNT_STATE.isEqualTo(
										DefaultAccountStatus.Unavailable,
									),
									ContextKeyExpr.has(
										"config.extensions.gallery.serviceUrl",
									),
								),
								group: "0_signin",
							},

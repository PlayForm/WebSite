	...
	implements IAuthenticationService
{
			...
			if (options?.issuer) {
				...
				if (
					!authProvider.issuers?.some(
						(i) =>
							i.toString(true) === issuerStr ||
							match(i.toString(true), issuerStr),
					)
				) {
					throw new Error(
	...
	async getOrActivateProviderIdForIssuer(
		issuer: URI,
		...
		for (const provider of providers) {
			const activeProvider = await this.tryActivateProvider(
				provider.id,
				true,
			);
			// Check the resolved issuers
			if (
				activeProvider.issuers?.some((i) =>
					match(i.toString(true), issuerStr),
				)
			) {

	...
	implements IWorkbenchExtensionManagementService
{
					...
					if (
						...
						untrustedPublishers.set(result.source.publisher, {
							...
							publisherDisplayName:
								result.source.publisherDisplayName,
						});
					}
				}
		...
		await Promise.all(
			extensions.map(async ({ extension, options }) => {
				try {
					...
					const servers =
						await this.getExtensionManagementServersToInstall(
							extension,
							manifest,
						);
					if (
	...
	private async checkForTrustedPublishers(
		...
		const getPublisherLink = ({
			publisherDisplayName,
			publisherLink,
		}: { publisherDisplayName: string; publisherLink?: string }) => {
